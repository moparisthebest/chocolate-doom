...
boolean I_MidiPipe_InitServer() {
  ...
  if (!CreatePipe(&midi_process_in_reader, &midi_process_in_writer, &sec_attrs,
                  0)) {
  ...
  if (!CreatePipe(&midi_process_out_reader, &midi_process_out_writer,
                  &sec_attrs, 0)) {
  ...
  // the executable name.
  M_snprintf(params_buf, sizeof(params_buf), "%d %Iu %Iu", snd_samplerate,
             (size_t)midi_process_in_reader, (size_t)midi_process_out_writer);
  cmdline =
      M_StringJoin(module, " \"" PACKAGE_STRING "\"", " ", params_buf, NULL);
  ...
  ok = CreateProcess(TEXT(module), TEXT(cmdline), NULL, NULL, TRUE, 0, NULL,
                     dirname, &startup_info, &proc_info);
