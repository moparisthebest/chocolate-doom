...
void A_PotteryExplode(mobj_t *actor) {
  ...
  if (actor->args[0]) { // Spawn an item
    if (!nomonsters ||
        !(mobjinfo[TranslateThingType[actor->args[0]]].flags &
          MF_COUNTKILL)) { // Only spawn monsters if not -nomonsters
  ...
  if (!netgame) {
    ...
    if (P_CheckSight(actor, pmo) &&
        (abs((int)R_PointToAngle2(pmo->x, pmo->y, actor->x, actor->y) -
             (int)pmo->angle) <=
         ANG45)) { // Previous state (pottery bit waiting state)
...
void A_LeafCheck(mobj_t *actor) {
  ...
  if (P_Random() > 64) {
    if (!actor->momx && !actor->momy) {
      P_ThrustMobj(actor, actor->target->angle, (P_Random() << 9) + FRACUNIT);
    }
    return;
  }
...
void GenerateOrbitTable(void)
{
        ...
        {
                orbitTableX[angle] = FixedMul(ORBIT_RADIUS,
finecosine[angle<<5]); orbitTableY[angle] = FixedMul(ORBIT_RADIUS,
finesine[angle<<5]);
        }
...
void A_HideThing(mobj_t *actor) { actor->flags2 |= MF2_DONTDRAW; }
...
void A_UnHideThing(mobj_t *actor) { actor->flags2 &= ~MF2_DONTDRAW; }
...
void A_PoisonBagInit(mobj_t *actor) {
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 28 * FRACUNIT, MT_POISONCLOUD);
  if (!mo) {
    return;
  }
...
void A_PoisonShroom(mobj_t *actor) { actor->tics = 128 + (P_Random() << 1); }
...
void A_CheckThrowBomb(mobj_t *actor) {
  if (abs(actor->momx) < 1.5 * FRACUNIT && abs(actor->momy) < 1.5 * FRACUNIT &&
      actor->momz < 2 * FRACUNIT && actor->state == &states[S_THROWINGBOMB6]) {
...
boolean A_LocalQuake(byte *args, mobj_t *actor) {
  ...
  do {
    ...
    if (target) {
      focus = P_SpawnMobj(target->x, target->y, target->z, MT_QUAKE_FOCUS);
      if (focus) {
        focus->args[0] = args[0];
...
void A_Quake(mobj_t *actor) {
  ...
  if (actor->args[1]-- > 0) {
    for (playnum = 0; playnum < maxplayers; playnum++) {
      ...
      victim = player->mo;
      dist = P_AproxDistance(actor->x - victim->x, actor->y - victim->y) >>
             (FRACBITS + 6);
...
void A_ThrustRaise(mobj_t *actor) {
  if (A_RaiseMobj(actor)) { // Reached it's target height
    actor->args[0] = 1;
    if (actor->args[1])
...
void A_ThrustBlock(mobj_t *actor) { actor->flags |= MF_SOLID; }
...
void A_SoAExplode(mobj_t *actor) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    mo = P_SpawnMobj(actor->x + ((r3 - 128) << 12),
                     actor->y + ((r2 - 128) << 12),
                     actor->z + (r1 * actor->height / 256), MT_ZARMORCHUNK);
...
void A_NoGravity(mobj_t *actor) { actor->flags |= MF_NOGRAVITY; }
