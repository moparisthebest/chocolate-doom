...
void P_AnimateSurfaces(void) {
  ...
  for (i = 0; i < AnimDefCount; i++) {
    ...
    if (ad->tics == 0) {
      ...
      if (ad->tics > 255) { // Random tics
        ad->tics = (ad->tics >> 16) + P_Random() % ((ad->tics & 0xff00) >> 8);
      }
...
static void P_LightningFlash(void) {
  ...
  if (LightningFlash) {
    ...
    } else { // remove the alternate lightning flash special
      tempLight = LightningLightLevels;
      tempSec = sectors;
      for (i = 0; i < numsectors; i++, tempSec++) {
        if (tempSec->ceilingpic == skyflatnum ||
...
void P_ForceLightning(void) { NextLightningFlash = 0; }
...
void P_InitLightning(void) {
  ...
  NextLightningFlash =
      ((P_Random() & 15) + 5) * 35; // don't flash at level start
}
