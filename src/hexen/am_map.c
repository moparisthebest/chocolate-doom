...
static byte antialias[NUMALIAS][8] = {{83, 84, 85, 86, 87, 88, 89, 90},
...
void AM_initVariables(void) {
  ...
  if (gameskill == sk_baby) {
    for (think = thinkercap.next; think != &thinkercap; think = think->next) {
      if (think->function != P_MobjThinker) { // not a mobj
        continue;
      }
    }
  }
...
void AM_loadPics(void) { maplump = W_CacheLumpName("AUTOPAGE", PU_STATIC); }
...
void AM_doFollowPlayer(void) {
  if (f_oldloc.x != plr->mo->x || f_oldloc.y != plr->mo->y) {
    ...
    /*
             ...
             if(f_oldloc.x == INT_MAX) //to eliminate an error when the user
       first dmapx=0;  //goes into the automap. mapxstart += dmapx; mapystart +=
       dmapy;
...
void AM_clearFB(int color) {
  ...
  if (followplayer) {
    ...
    oldplr.x = plr->mo->x;
    oldplr.y = plr->mo->y;
    //              if(f_oldloc.x == INT_MAX) //to eliminate an error when the
    //              user first
...
void PUTDOT(short xx, short yy, byte *cc, byte *cm) {
  ...
  if (cc > cm && cm != NULL) {
    cc = cm;
             ...
             oldcc + 6) // don't let the color escape from the fade table...
  {
    cc = oldcc + 6;
  }
  if (yy == oldyy + 1) {
      ...
      if (!lines[i].backsector) {
        ...
        if (lines[i].flags & ML_SECRET) // secret door
        {
          if (cheating)
            ...
            AM_drawMline(&l, WALLCOLORS + lightlev);
        } else if (lines[i].special == 13 ||
                   lines[i].special ==
                       83) { // Locked door line -- all locked doors are greed
          AM_drawMline(&l, GREENKEY);
        } else if (lines[i].special == 70 ||
                   lines[i].special == 71) { // intra-level teleports are blue
          AM_drawMline(&l, BLUEKEY);
        } else if (lines[i].special == 74 ||
                   lines[i].special ==
                       75) { // inter-level teleport/game-winning exit -- both
                             // are red
...
void AM_drawPlayers(void) {
  int i;
  player_t *p;
  static int their_colors[] = {AM_PLR1_COLOR, AM_PLR2_COLOR, AM_PLR3_COLOR,
                               AM_PLR4_COLOR, AM_PLR5_COLOR, AM_PLR6_COLOR,
                               AM_PLR7_COLOR, AM_PLR8_COLOR};
  int their_color = -1;
  int color;
...
void AM_drawkeys(void)
{
        ...
        {
                AM_drawLineCharacter(keysquare, NUMKEYSQUARELINES, 0, 0,
YELLOWKEY, KeyPoints[0].x, KeyPoints[0].y);
        }
        if(KeyPoints[1].x != 0 || KeyPoints[1].y != 0)
        {
                AM_drawLineCharacter(keysquare, NUMKEYSQUARELINES, 0, 0,
GREENKEY, KeyPoints[1].x, KeyPoints[1].y);
        }
        if(KeyPoints[2].x != 0 || KeyPoints[2].y != 0)
        {
                AM_drawLineCharacter(keysquare, NUMKEYSQUARELINES, 0, 0,
BLUEKEY, KeyPoints[2].x, KeyPoints[2].y);
        }
}
*/
...
void AM_DrawDeathmatchStats(void) {
  ...
  for (i = 0; i < maxplayers; i++) {
    if (!playeringame[order[i]]) {
      ...
      M_snprintf(textBuffer, sizeof(textBuffer), "%d", fragCount[order[i]]);
...
static void DrawWorldTimer(void) {
  ...
  M_snprintf(timeBuffer, sizeof(timeBuffer), "%.2d : %.2d : %.2d", hours,
             minutes, seconds);
