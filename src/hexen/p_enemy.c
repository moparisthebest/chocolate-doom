...
void P_RecursiveSound(sector_t *sec, int soundblocks) {
  ...
  if (sec->validcount == validcount &&
      sec->soundtraversed <= soundblocks + 1) { // Already flooded
    return;
  }
...
boolean P_CheckMeleeRange(mobj_t *actor) {
  ...
  if (mo->z > actor->z + actor->height) { // Target is higher than the attacker
    return (false);
  } else if (actor->z > mo->z + mo->height) { // Attacker is higher
    return (false);
  }
  return (true);
}
...
boolean P_CheckMissileRange(mobj_t *actor) {
  ...
  if (actor->reactiontime) { // Don't attack yet
    return (false);
  }
  dist = (P_AproxDistance(actor->x - actor->target->x,
          ...
          FRACBITS) -
         64;
  if (!actor->info->meleestate) { // No melee attack, so fire more frequently
    dist -= 128;
  }
  if (dist > 200) {
    dist = 200;
...
boolean P_LookForMonsters(mobj_t *actor) {
  ...
  if (!P_CheckSight(players[0].mo, actor)) { // Player can't see monster
    return (false);
  }
  count = 0;
  for (think = thinkercap.next; think != &thinkercap; think = think->next) {
    if (think->function != P_MobjThinker) { // Not a mobj thinker
      continue;
    }
    mo = (mobj_t *)think;
    if (!(mo->flags & MF_COUNTKILL) || (mo == actor) ||
        (mo->health <= 0)) { // Not a valid monster
      continue;
    }
    if (P_AproxDistance(actor->x - mo->x, actor->y - mo->y) >
        MONS_LOOK_RANGE) { // Out of range
      continue;
    }
    if (P_Random() < 16) { // Skip
      continue;
    }
    if (count++ > MONS_LOOK_LIMIT) { // Stop searching
      return (false);
    }
    if (!P_CheckSight(actor, mo)) { // Out of sight
      continue;
    }
    if (actor->type == MT_MINOTAUR) {
      if ((mo->type == MT_MINOTAUR) && (mo->target != actor->special1.p->mo)) {
        continue;
      }
    }
...
boolean P_LookForPlayers(mobj_t *actor, boolean allaround) {
  ...
  if (!netgame &&
      players[0].health <=
          0) { // Single player game and player is dead, look for monsters
  ...
  for (;; actor->lastlook = (actor->lastlook + 1) % maxplayers) {
    ...
    if (player->mo->flags & MF_SHADOW) { // Player is invisible
      if ((P_AproxDistance(player->mo->x - actor->x, player->mo->y - actor->y) >
           2 * MELEERANGE) &&
          P_AproxDistance(player->mo->momx, player->mo->momy) <
              5 * FRACUNIT) { // Player is sneaking - can't detect
        return (false);
      }
      if (P_Random() < 225) { // Player isn't sneaking, but still didn't detect
        return (false);
      }
    }
    if (actor->type == MT_MINOTAUR) {
...
seeyou:
  if (actor->info->seesound) {
    ...
    if (actor->flags2 & MF2_BOSS) { // Full volume
...
void A_Chase(mobj_t *actor) {
  ...
  if (gameskill == sk_nightmare) { // Monsters move faster in nightmare mode
  ...
  if (!actor->target ||
      !(actor->target->flags & MF_SHOOTABLE)) { // look for a new target
    if (P_LookForPlayers(actor, true)) {        // got a new target
      return;
    }
...
void A_FaceTarget(mobj_t *actor) {
  ...
  if (actor->target->flags & MF_SHADOW) { // Target is a ghost
...
void A_SetInvulnerable(mobj_t *actor) { actor->flags2 |= MF2_INVULNERABLE; }
...
boolean P_UpdateMorphedMonster(mobj_t *actor, int tics) {
  ...
  if (P_TestMobjLocation(mo) == false) { // Didn't fit
    P_RemoveMobj(mo);
...
//      args[0]                 args[0]-args[3] together make up minotaur start
//      time args[1]                 | args[2]                 | args[3] V
  ...
  if (!actor->target) // Normal monster search
  {
    for (think = thinkercap.next; think != &thinkercap; think = think->next) {
      ...
      if ((mo == master) || (mo == actor))
        continue;
      if ((mo->type == MT_MINOTAUR) && (mo->special1.m == actor->special1.m))
        continue;
      actor->target = mo;
...
void A_MinotaurAtk3(mobj_t *actor) {
  ...
  if (P_CheckMeleeRange(actor)) {
    P_DamageMobj(actor->target, actor, actor, HITDICE(3));
    if ((player = actor->target->player) != NULL) { // Squish the player
...
void A_Scream(mobj_t *actor) {
  ...
  if (actor->player) {
    if (actor->player->morphTics) {
      ...
      if (actor->momz <= -39 * FRACUNIT) { // Falling splat
        sound = SFX_PLAYER_FALLING_SPLAT;
      } else if (actor->health > -50) { // Normal death sound
      ...
      } else if (actor->health > -100) { // Crazy death sound
        switch (actor->player->class) {
        case PCLASS_FIGHTER:
...
void A_SkullPop(mobj_t *actor) {
  ...
  actor->flags &= ~MF_SOLID;
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 48 * FRACUNIT, MT_BLOODYSKULL);
  // mo->target = actor;
...
void A_QueueCorpse(mobj_t *actor) {
  ...
  if (corpseQueueSlot >=
      CORPSEQUEUESIZE) { // Too many corpses - remove an old one
...
void A_AddPlayerCorpse(mobj_t *actor) {
  if (bodyqueslot >=
      BODYQUESIZE) { // Too many player corpses - remove an old one
...
void A_SerpentRaiseHump(mobj_t *actor) { actor->floorclip -= 4 * FRACUNIT; }
...
void A_SerpentLowerHump(mobj_t *actor) { actor->floorclip += 4 * FRACUNIT; }
...
void A_SerpentHumpDecide(mobj_t *actor) {
  if (actor->type == MT_SERPENTLEADER) {
    if (P_Random() > 30) {
      return;
    } else if (P_Random() < 40) { // Missile attack
  ...
  if (!P_CheckMeleeRange(
          actor)) { // The hump shouldn't occur when within melee range
...
void A_SerpentSpawnGibs(mobj_t *actor) {
  ...
  r1 = P_Random();
  r2 = P_Random();
  mo = P_SpawnMobj(actor->x + ((r2 - 128) << 12), actor->y + ((r1 - 128) << 12),
...
void A_FloatGib(mobj_t *actor) { actor->floorclip -= FRACUNIT; }
...
void A_SinkGib(mobj_t *actor) { actor->floorclip += FRACUNIT; }
...
void A_DelayGib(mobj_t *actor) { actor->tics -= P_Random() >> 2; }
...
void A_BishopPuff(mobj_t *actor) {
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 40 * FRACUNIT, MT_BISHOP_PUFF);
  if (mo) {
    mo->momz = FRACUNIT / 2;
  }
}
...
void A_BishopPainBlur(mobj_t *actor) {
  ...
  mo = P_SpawnMobj(actor->x + (r3 << 12), actor->y + (r2 << 12),
                   actor->z + (r1 << 11), MT_BISHOPPAINBLUR);
  if (mo) {
    mo->angle = actor->angle;
  }
}
...
static void DragonSeek(mobj_t *actor, angle_t thresh, angle_t turnMax) {
  ...
  if (target->flags & MF_SHOOTABLE &&
    ...
    if (abs((int)actor->angle -
            (int)R_PointToAngle2(actor->x, actor->y, target->x, target->y)) <
        ANG45 / 2) {
  ...
  if (dist < 4) { // Hit the target thing
    if (actor->target && P_Random() < 200) {
      bestArg = -1;
      bestAngle = ANG_MAX;
      angleToTarget = R_PointToAngle2(actor->x, actor->y, actor->target->x,
                                      actor->target->y);
      for (i = 0; i < 5; i++) {
        if (!target->args[i]) {
          continue;
      ...
      if (bestArg != -1) {
        search = -1;
        actor->special1.m = P_FindMobjFromTID(target->args[bestArg], &search);
      }
    } else {
      do {
        i = (P_Random() >> 2) % 5;
      ...
      search = -1;
      actor->special1.m = P_FindMobjFromTID(target->args[i], &search);
    }
  }
}
...
void A_DragonFlight(mobj_t *actor) {
  ...
  if (actor->target) {
    if (!(actor->target->flags & MF_SHOOTABLE)) { // target died
      actor->target = NULL;
      return;
    }
    angle =
...
void A_DragonFX2(mobj_t *actor) {
  ...
  for (i = 1 + (P_Random() & 3); i; i--) {
    ...
    mo = P_SpawnMobj(actor->x + ((r3 - 128) << 14),
                     actor->y + ((r2 - 128) << 14),
                     actor->z + ((r1 - 128) << 12), MT_DRAGON_FX2);
    if (mo) {
...
void A_DemonDeath(mobj_t *actor) {
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 45 * FRACUNIT, MT_DEMONCHUNK1);
  if (mo) {
    angle = actor->angle + ANG90;
    mo->momz = 8 * FRACUNIT;
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 45 * FRACUNIT, MT_DEMONCHUNK2);
  if (mo) {
    angle = actor->angle - ANG90;
    mo->momz = 8 * FRACUNIT;
    mo->momx = FixedMul((P_Random() << 10) + FRACUNIT,
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 45 * FRACUNIT, MT_DEMONCHUNK3);
  if (mo) {
    angle = actor->angle - ANG90;
    mo->momz = 8 * FRACUNIT;
    mo->momx = FixedMul((P_Random() << 10) + FRACUNIT,
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 45 * FRACUNIT, MT_DEMONCHUNK4);
  if (mo) {
    angle = actor->angle - ANG90;
    mo->momz = 8 * FRACUNIT;
    mo->momx = FixedMul((P_Random() << 10) + FRACUNIT,
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 45 * FRACUNIT, MT_DEMONCHUNK5);
  if (mo) {
    angle = actor->angle - ANG90;
    mo->momz = 8 * FRACUNIT;
    mo->momx = FixedMul((P_Random() << 10) + FRACUNIT,
...
void A_IceGuyLook(mobj_t *actor) {
  ...
  if (P_Random() < 64) {
    ...
    P_SpawnMobj(actor->x + FixedMul(dist, finecosine[an]),
                ...
                actor->z + 60 * FRACUNIT, MT_ICEGUY_WISP1 + (P_Random() & 1));
  }
}
...
//
// Sorcerer Variables
//              special1                Angle of ball 1 (all others relative to
//              that) special2                which ball to stop at in stop mode
//              (MT_???) args[0]                 Denfense time args[1] Number of
//              full rotations since stopping mode args[2] Target orbit speed
//              for acceleration/deceleration args[3]                 Movement
//              mode (see SORC_ macros) args[4]                 Current ball
//              orbit speed
//      Sorcerer Ball Variables
//              special1                Previous angle of ball (for woosh)
//              special2                Countdown of rapid fire (FX4)
//              args[0]                 If set, don't play the bounce sound when
//              bouncing
//============================================================================
...
void A_CastSorcererSpell(mobj_t *actor) {
  ...
  case MT_SORCBALL3: // Reinforcements
    ...
    if (actor->health < (actor->info->spawnhealth / 3)) { // Spawn 2 at a time
...
void A_FreezeDeathChunks(mobj_t *actor) {
  ...
  for (i = 12 + (P_Random() & 15); i >= 0; i--) {
    ...
    mo = P_SpawnMobj(actor->x + (((r3 - 128) * actor->radius) >> 7),
                     ...
                     actor->z + (r1 * actor->height / 255), MT_ICECHUNK);
  ...
  if (actor->player) { // attach the player's view to a chunk of ice
...
void A_KoraxBonePop(mobj_t *actor) {
  ...
  mo = P_SpawnMissileAngle(actor, MT_KORAX_SPIRIT1, ANG60 * 0, 5 * FRACUNIT);
  if (mo)
    KSpiritInit(mo, actor);
  mo = P_SpawnMissileAngle(actor, MT_KORAX_SPIRIT2, ANG60 * 1, 5 * FRACUNIT);
  if (mo)
    KSpiritInit(mo, actor);
  mo = P_SpawnMissileAngle(actor, MT_KORAX_SPIRIT3, ANG60 * 2, 5 * FRACUNIT);
  if (mo)
    KSpiritInit(mo, actor);
  mo = P_SpawnMissileAngle(actor, MT_KORAX_SPIRIT4, ANG60 * 3, 5 * FRACUNIT);
  if (mo)
    KSpiritInit(mo, actor);
  mo = P_SpawnMissileAngle(actor, MT_KORAX_SPIRIT5, ANG60 * 4, 5 * FRACUNIT);
  if (mo)
    KSpiritInit(mo, actor);
  mo = P_SpawnMissileAngle(actor, MT_KORAX_SPIRIT6, ANG60 * 5, 5 * FRACUNIT);
  if (mo)
    KSpiritInit(mo, actor);
