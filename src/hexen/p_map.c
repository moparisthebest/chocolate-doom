...
(block movement, be shot, etc) will also need to be linked into the blockmap. If
the thing has the MF_NOBLOCK flag set, it will not use the block links. It can
still interact with other things, but only as the instigator (missiles will run
into other things, but nothing can run into a missile).   Each block in the grid
is 128*128 units, and knows about every line_t that it contains a piece of, and
every interactable mobj_t that has it's origin contained.

A valid mobj_t is a mobj_t that has the proper subsector_t filled in for it's xy
coordinates and is linked into the subsector's sector or has the MF_NOSECTOR
...
boolean PIT_StompThing(mobj_t *thing) {
  ...
  if (!(tmthing->flags2 & MF2_TELESTOMP)) { // Not allowed to stomp things
    return (false);
  }
...
boolean PIT_CheckLine(line_t *ld) {
  ...
  */
  ...
  if (ld->special) { // Contacted a special line, add it to the list
...
boolean PIT_CheckThing(mobj_t *thing) {
  ...
  if (!(thing->flags &
        (MF_SOLID | MF_SPECIAL | MF_SHOOTABLE))) { // Can't hit thing
    return (true);
  }
  blockdist = thing->radius + tmthing->radius;
  if (abs(thing->x - tmx) >= blockdist ||
      abs(thing->y - tmy) >= blockdist) { // Didn't hit thing
    return (true);
  }
  if (thing == tmthing) { // Don't clip against self
    return (true);
  }
  BlockingMobj = thing;
  if (tmthing->flags2 &
      MF2_PASSMOBJ) { // check if a mobj passed over/under another object
    if (tmthing->type == MT_BISHOP &&
        thing->type == MT_BISHOP) { // don't let bishops fly over other bishops
      return false;
    }
    if (tmthing->z >= thing->z + thing->height &&
      ...
      return (true);
               ...
               !(thing->flags & MF_SPECIAL)) { // under thing
      return (true);
    }
  }
    ...
    if (tmthing->type == MT_MINOTAUR) {
      ...
      if (thing->flags & MF_SHOOTABLE && thing != tmthing->target) {
        if (netgame && !deathmatch &&
            thing->player) { // don't attack other co-op players
          return true;
        }
        ...
        if (P_Random() < 96) {
          ...
          if (P_Random() < 128) {
            P_SpawnMobj(tmthing->x, tmthing->y, tmthing->z, MT_HOLY_PUFF);
  ...
  if (tmthing->flags & MF_MISSILE) {
    ...
    if (tmthing->z > thing->z + thing->height) { // Over thing
      return (true);
    }
    if (tmthing->z + tmthing->height < thing->z) { // Under thing
      return (true);
    }
    ...
    if (tmthing->type == MT_LIGHTNING_FLOOR ||
      ...
      if (thing->flags & MF_SHOOTABLE && thing != tmthing->target) {
        ...
        if ((!thing->player && !(thing->flags2 & MF2_BOSS)) ||
          ...
          if (thing->type == MT_CENTAUR ||
              thing->type ==
                  MT_CENTAURLEADER) { // Lightning does more damage to centaurs
    ...
    if (tmthing->target &&
      ...
      if (thing == tmthing->target) { // Don't missile self
        return (true);
      }
      if (!thing
               ->player) { // Hit same species as originator, explode, no damage
        return (false);
      }
    }
    if (!(thing->flags & MF_SHOOTABLE)) { // Didn't do any damage
      return !(thing->flags & MF_SOLID);
    }
    if (tmthing->flags2 & MF2_RIP) {
      if (!(thing->flags & MF_NOBLOOD) && !(thing->flags2 & MF2_REFLECTIVE) &&
          !(thing->flags2 & MF2_INVULNERABLE)) { // Ok to spawn some blood
      ...
      if (thing->flags2 & MF2_PUSHABLE &&
          !(tmthing->flags2 & MF2_CANNOTPUSH)) { // Push thing
  ...
  if (thing->flags & MF_SPECIAL) {
    ...
    if (tmflags & MF_PICKUP) {             // Can be picked up by tmthing
...
boolean PIT_CheckOnmobjZ(mobj_t *thing) {
  ...
  if (tmthing->z > thing->z + thing->height) {
    return (true);
  } else if (tmthing->z + tmthing->height < thing->z) { // under thing
    return (true);
  }
...
boolean P_TestMobjLocation(mobj_t *mobj) {
  ...
  if (P_CheckPosition(mobj, mobj->x, mobj->y)) { // XY is ok, now check Z
    mobj->flags = flags;
    if ((mobj->z < mobj->floorz) ||
        (mobj->z + mobj->height > mobj->ceilingz)) { // Bad Z
      return (false);
    }
    return (true);
  }
...
boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y) {
  ...
  floatok = false;
  if (!P_CheckPosition(thing, x, y)) { // Solid wall or thing
  ...
  if (!(thing->flags & MF_NOCLIP)) {
    ...
    if (!(thing->flags & MF_TELEPORT) &&
        ...
        !(thing->flags2 & MF2_FLY)) { // mobj must lower itself to fit
      goto pushline;
    }
    ...
    if (!(thing->flags & (MF_DROPOFF | MF_FLOAT)) &&
        ...
        !(thing->flags2 &
          MF2_BLASTED)) { // Can't move over a dropoff unless it's been blasted
      return (false);
    }
    if (thing->flags2 & MF2_CANTLEAVEFLOORPIC &&
        (tmfloorpic != thing->subsector->sector->floorpic ||
         tmfloorz - thing->z !=
             0)) { // must stay within a sector of a certain floor type
      return false;
    }
  }
...
retry:
  ...
  //
  if (bestslidefrac ==
      FRACUNIT + 1) { // the move must have hit the middle, so stairstep
  stairstep:
...
boolean PTR_AimTraverse(intercept_t *in) {
  ...
  if (!(th->flags & MF_SHOOTABLE)) { // corpse or something
    return true;
  }
  if (th->player && netgame &&
      !deathmatch) { // don't aim at fellow co-op players
    return true;
  }
...
boolean PTR_ShootTraverse(intercept_t *in) {
  ...
  */
  ...
  if (la_damage) {
    ...
    if (PuffType == MT_FLAMEPUFF2) { // Cleric FlameStrike does fire damage
...
boolean PTR_PuzzleItemTraverse(intercept_t *in) {
  ...
  if (in->isaline) { // Check line
    ...
    if (P_PointOnLineSide(PuzzleItemUser->x, PuzzleItemUser->y,
                          in->d.line) == 1) { // Don't use back sides
      return false;
    }
    if (PuzzleItemType != in->d.line->arg1) { // Item type doesn't match
      return false;
    }
  ...
  // Check thing
  mobj = in->d.thing;
  if (mobj->special != USE_PUZZLE_ITEM_SPECIAL) { // Wrong special
    return true;
  }
  if (PuzzleItemType != mobj->args[0]) { // Item type doesn't match
    return true;
  }
...
boolean PIT_RadiusAttack(mobj_t *thing) {
  ...
  //      }
  if (!DamageSource &&
      thing == bombsource) { // don't damage the source of the explosion
    return true;
  }
  if (abs((thing->z - bombspot->z) >> FRACBITS) >
      2 * bombdistance) { // too high/low
    return true;
  }
  ...
  if (dist >= bombdistance) { // Out of range
    return true;
  }
  if (P_CheckSight(thing, bombspot)) { // OK to damage, target is in direct path
