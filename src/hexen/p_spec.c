...
struct {
  const char *name;
  int type;
} TerrainTypeDefs[] = {{"X_005", FLOOR_WATER},
                       {"X_001", FLOOR_LAVA},
                       {"X_009", FLOOR_SLUDGE},
                       {"F_033", FLOOR_ICE},
                       {"END", -1}};
...
boolean P_ExecuteLineSpecial(int special, byte *args, line_t *line, int side,
  ...
  case 75:           // Teleport_EndGame
    if (side == 0) { // Only teleport when crossing the front side of a line
      ...
      {
        buttonSuccess = true;
        if (deathmatch) { // Winning in deathmatch just goes back to map 1
          G_Completed(1, 0);
        } else { // Passing -1, -1 to G_Completed() starts the Finale
          G_Completed(-1, -1);
        }
      }
    }
...
boolean P_ActivateLine(line_t *line, mobj_t *mo, int side, int activationType) {
  ...
  if (!mo->player && !(mo->flags & MF_MISSILE)) {
    if (lineActivation != SPAC_MCROSS) { // currently, monsters can only
                                         // activate the MCROSS activation type
      return false;
    }
  ...
  if (!repeat &&
      buttonSuccess) { // clear the special on non-retriggerable lines
    line->special = 0;
  }
...
void P_PlayerInSpecialSector(player_t *player) {
  sector_t *sector;
  static int pushTab[3] = {2048 * 5, 2048 * 10, 2048 * 25};
...
void P_PlayerOnSpecialFlat(player_t *player, int floorType) {
  if (player->mo->z != player->mo->floorz) { // Player is not touching the floor
    return;
  }
  switch (floorType) {
  case FLOOR_LAVA:
