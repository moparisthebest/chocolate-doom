...
static sector_t *DequeueStairSector(int *type, int *height) {
  ...
  if (QueueHead == QueueTail) { // queue is empty
    return NULL;
  }
...
int EV_BuildPillar(line_t *line, byte *args, boolean crush) {
  ...
  while ((secnum = P_FindSectorFromTag(args[0], secnum)) >= 0) {
    ...
    if (sec->floorheight == sec->ceilingheight) { // pillar is already closed
      continue;
    }
    rtn = 1;
    if (!args[2]) {
      newHeight =
          sec->floorheight + ((sec->ceilingheight - sec->floorheight) / 2);
    } else {
...
int EV_OpenPillar(line_t *line, byte *args) {
  ...
  while ((secnum = P_FindSectorFromTag(args[0], secnum)) >= 0) {
    ...
    if (sec->specialdata)
      continue;                                   // already moving
    if (sec->floorheight != sec->ceilingheight) { // pillar isn't closed
      continue;
    }
    rtn = 1;
    pillar = Z_Malloc(sizeof(*pillar), PU_LEVSPEC, 0);
...
void T_FloorWaggle(floorWaggle_t *waggle) {
  ...
  case WGLSTATE_REDUCE:
    if ((waggle->scale -= waggle->scaleDelta) <= 0) { // Remove
...
boolean EV_StartFloorWaggle(int tag, int height, int speed, int offset,
  ...
  while ((sectorIndex = P_FindSectorFromTag(tag, sectorIndex)) >= 0) {
    ...
    if (sector->specialdata) { // Already busy with another thinker
      continue;
    }
    retCode = true;
