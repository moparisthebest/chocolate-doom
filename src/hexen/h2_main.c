...
static void D_HexenQuitMessage(void) { printf("\nHexen: Beyond Heretic\n"); }
...
static void DrawMessage(void) {
  ...
  if (player->messageTics <= 0) { // No message
    return;
  }
  if (player->yellowMessage) {
    MN_DrTextAYellow(player->message, 160 - MN_TextAWidth(player->message) / 2,
                     1);
  } else {
...
void H2_AdvanceDemo(void) { advancedemo = true; }
...
static void CreateSavePath(void) { M_MakeDirectory(SavePath); }
