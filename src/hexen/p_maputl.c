...
void P_UnsetThingPosition(mobj_t *thing) {
  ...
  if (!(thing->flags & MF_NOSECTOR)) { // inert things don't need to be in
                                       // blockmap unlink from subsector
    if (thing->snext)
  ...
  if (!(thing->flags & MF_NOBLOCKMAP)) { // inert things don't need to be in
                                         // blockmap unlink from block map
    if (thing->bnext)
...
void P_SetThingPosition(mobj_t *thing) {
  ...
  thing->subsector = ss;
  if (!(thing->flags &
        MF_NOSECTOR)) { // invisible things don't go into the sector links
    sec = ss->sector;
  ...
  //
  if (!(thing->flags &
        MF_NOBLOCKMAP)) { // inert things don't need to be in blockmap
    blockx = (thing->x - bmaporgx) >> MAPBLOCKSHIFT;
    blocky = (thing->y - bmaporgy) >> MAPBLOCKSHIFT;
    if (blockx >= 0 && blockx < bmapwidth && blocky >= 0 &&
        blocky < bmapheight) {
...
mobj_t *P_RoughMonsterSearch(mobj_t *mo, int distance) {
  ...
  if (startX >= 0 && startX < bmapwidth && startY >= 0 && startY < bmapheight) {
    ...
    if (target != NULL) { // found a target right away
      return target;
    }
  }
...
static mobj_t *RoughBlockCheck(mobj_t *mo, int index) {
  ...
  while (link) {
    ...
    } else if (mo->type == MT_MINOTAUR) // looking around minotaur
    {
      ...
      if ((link->flags & MF_COUNTKILL) || (link->player && (link != master))) {
        if (!(link->flags & MF_SHOOTABLE)) {
          link = link->bnext;
          continue;
        }
    ...
    } else if (mo->type == MT_MSTAFF_FX2) // bloodscourge
    {
      if ((link->flags & MF_COUNTKILL ||
        ...
        if (netgame && !deathmatch && link->player) {
          ...
          master = mo->target;
          angle = R_PointToAngle2(master->x, master->y, link->x, link->y) -
                  master->angle;
          angle >>= 24;
