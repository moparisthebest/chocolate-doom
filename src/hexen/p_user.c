...
int PStateNormal[NUMCLASSES] = {S_FPLAY, S_CPLAY, S_MPLAY, S_PIGPLAY};

int PStateRun[NUMCLASSES] = {S_FPLAY_RUN1, S_CPLAY_RUN1, S_MPLAY_RUN1,
                             S_PIGPLAY_RUN1};

int PStateAttack[NUMCLASSES] = {S_FPLAY_ATK1, S_CPLAY_ATK1, S_MPLAY_ATK1,
                                S_PIGPLAY_ATK1};

int PStateAttackEnd[NUMCLASSES] = {S_FPLAY_ATK2, S_CPLAY_ATK3, S_MPLAY_ATK2,
                                   S_PIGPLAY_ATK1};
  ...
  if (fly && player->powers[pw_flight]) {
    if (fly != TOCENTER) {
      ...
      if (!(player->mo->flags2 & MF2_FLY)) {
        ...
        if (player->mo->momz <= -39 * FRACUNIT) { // stop falling scream
...
void P_DeathThink(player_t *player) {
  ...
  if (player->mo->type == MT_BLOODYSKULL ||
      player->mo->type ==
          MT_ICECHUNK) { // Flying bloody skull or flying ice chunk
  ...
  } else if (!(player->mo->flags2 &
               MF2_ICEDAMAGE)) { // Fall to ground (if not frozen)
    player->deltaviewheight = 0;
  ...
  if (player->attacker && player->attacker != player->mo) { // Watch killer
    dir = P_FaceMobj(player->mo, player->attacker, &delta);
    if (delta <
        ANG1 * 10) { // Looking at killer, so fade damage and poison counters
...
void P_MorphPlayerThink(player_t *player) {
  ...
  pmo = player->mo;
  if (!(pmo->momx + pmo->momy) && P_Random() < 64) { // Snout sniff
...
boolean P_UndoPlayerMorph(player_t *player) {
  ...
  } else if (playerNum) { // Set color translation bits for player sprites
    mo->flags |= playerNum << MF_TRANSSHIFT;
  }
  mo->angle = angle;
  mo->player = player;
...
void P_PlayerThink(player_t *player) {
  ...
  cmd = &player->cmd;
  if (player->mo->flags &
      MF_JUSTATTACKED) { // Gauntlets attack auto forward motion
    cmd->angleturn = 0;
  ...
  if (!player->messageTics ||
      player->messageTics ==
          -1) { // Refresh the screen when a message goes away
  ...
  if (cmd->arti) { // Use an artifact
    ...
    if (cmd->arti ==
        NUMARTIFACTS) { // use one of each artifact (except puzzle artifacts)
      int i;
  ...
  if (cmd->buttons & BT_SPECIAL) { // A special event has no other buttons
    cmd->buttons = 0;
  }
  ...
  // Morph counter
  if (player->morphTics) {
    if (!--player->morphTics) { // Attempt to undo the pig
    ...
    if (player->powers[pw_infrared] <= BLINKTHRESHOLD) {
      ...
      if (newtorch) {
        ...
        newtorchdelta = (newtorch == player->fixedcolormap)
                            ? 0
                            : ((newtorch > player->fixedcolormap) ? 1 : -1);
      }
    }
  } else {
    player->fixedcolormap = 0;
...
void P_ArtiTele(player_t *player) {
  ...
  if (player->morphTics) { // Teleporting away will undo any morph effects (pig)
    P_UndoPlayerMorph(player);
  }
...
void P_TeleportOther(mobj_t *victim) {
  if (victim->player) {
    ...
    if (victim->flags & MF_COUNTKILL && victim->special) {
      ...
      P_ExecuteLineSpecial(victim->special, victim->args, NULL, 0, victim);
      victim->special = 0;
    }
...
void P_BlastRadius(player_t *player) {
  ...
  for (think = thinkercap.next; think != &thinkercap; think = think->next) {
    ...
    mo = (mobj_t *)think;
    if ((mo == pmo) || (mo->flags2 & MF2_BOSS)) { // Not a valid monster
      continue;
    }
        ...
        (mo->flags & MF_ICECORPSE))     // frozen corpse
    {
      ...
      continue;
               ...
               !(mo->flags &
                 MF_MISSILE)) { // Must be monster, player, or missile
      continue;
    }
    ...
    if (dist > BLAST_RADIUS_DIST) { // Out of range
      continue;
    }
...
boolean P_HealRadius(player_t *player) {
  ...
  for (think = thinkercap.next; think != &thinkercap; think = think->next) {
    ...
    if (mo->health <= 0)
      continue;
    dist = P_AproxDistance(pmo->x - mo->x, pmo->y - mo->y);
    if (dist > HEAL_RADIUS_DIST) { // Out of range
      continue;
    }
...
void P_PlayerRemoveArtifact(player_t *player, int slot) {
  ...
  if (!(--player->inventory[slot]
              .count)) { // Used last of a type - compact the artifact list
    ...
    if (player == &players[consoleplayer]) { // Set position markers and get
                                             // next readyArtifact
      inv_ptr--;
      if (inv_ptr < 6) {
        curpos--;
...
void P_PlayerUseArtifact(player_t *player, artitype_t arti) {
  ...
  for (i = 0; i < player->inventorySlotNum; i++) {
    if (player->inventory[i].type == arti) { // Found match - try to use
      if (P_UseArtifact(player,
                        arti)) { // Artifact was used - remove it from inventory
      ...
      } else if (arti < arti_firstpuzzitem) { // Unable to use artifact, advance
                                              // pointer
...
boolean P_UseArtifact(player_t *player, artitype_t arti) {
  ...
  case arti_fly:
    ...
    if (player->mo->momz <= -35 * FRACUNIT) { // stop falling scream
      S_StopSound(player->mo);
    }
    break;
  case arti_summon:
