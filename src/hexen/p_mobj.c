...
boolean P_SetMobjState(mobj_t *mobj, statenum_t state) {
  ...
  mobj->frame = st->frame;
  if (st->action) { // Call action function
    st->action(mobj);
  }
  return (true);
}
...
boolean P_SeekerMissile(mobj_t *actor, angle_t thresh, angle_t turnMax) {
  ...
  if (actor->z + actor->height < target->z ||
      target->z + target->height < actor->z) { // Need to seek vertically
...
void P_XYMovement(mobj_t *mo) {
  ...
  if (!mo->momx && !mo->momy) {
    if (mo->flags & MF_SKULLFLY) { // A flying mobj slammed into something
  ...
  do {
    ...
    if (!P_TryMove(mo, ptryx, ptryy)) { // Blocked move
      if (mo->flags2 & MF2_SLIDE) {     // Try to slide along it
        if (BlockingMobj == NULL) {     // Slide against wall
          P_SlideMove(mo);
        } else { // Slide against mobj
        ...
        if (mo->flags2 & MF2_FLOORBOUNCE) {
          if (BlockingMobj) {
            if ((BlockingMobj->flags2 & MF2_REFLECTIVE) ||
              ...
              angle = R_PointToAngle2(BlockingMobj->x, BlockingMobj->y, mo->x,
                                      mo->y) +
                      ANG1 * ((P_Random() % 16) - 8);
      ...
      explode:
        ...
        if (ceilingline && ceilingline->backsector &&
            ceilingline->backsector->ceilingpic ==
                skyflatnum) { // Hack to prevent missiles exploding against the
                              // sky
  ...
  if (player &&
      player->cheats & CF_NOMOMENTUM) { // Debug option for no sliding at all
    mo->momx = mo->momy = 0;
    return;
  }
  if (mo->flags & (MF_MISSILE | MF_SKULLFLY)) { // No friction for missiles
    return;
  }
  if (mo->z > mo->floorz && !(mo->flags2 & MF2_FLY) &&
      !(mo->flags2 & MF2_ONMOBJ)) { // No friction when falling
    if (mo->type != MT_BLASTEFFECT)
      return;
  }
  if (mo->flags & MF_CORPSE) { // Don't stop sliding if halfway off a step with
                               // some momentum
  ...
  if (mo->momx > -STOPSPEED && mo->momx < STOPSPEED && mo->momy > -STOPSPEED &&
      ...
      (!player ||
       (player->cmd.forwardmove == 0 &&
        player->cmd.sidemove == 0))) { // If in a walking frame, stop moving
    if (player) {
      if ((unsigned)((player->mo->state - states) - PStateRun[player->class]) <
          4) {
...
void P_MonsterFallingDamage(mobj_t *mo) {
  ...
  mom = abs(mo->momz);
  if (mom > 35 * FRACUNIT) { // automatic death
    damage = 10000;
  } else {
  ...
  if (mo->z <= mo->floorz) { // Hit the floor
    if (mo->flags & MF_MISSILE) {
      ...
      if (mo->flags2 & MF2_FLOORBOUNCE) {
        ...
        return;
      } else if (mo->type == MT_HOLY_FX) { // The spirit struck the ground
        mo->momz = 0;
        P_HitFloor(mo);
        return;
      } else if (mo->type == MT_MNTRFX2 ||
                 mo->type == MT_LIGHTNING_FLOOR) { // Minotaur floor fire can go
                                                   // up steps
        return;
      } else {
        P_HitFloor(mo);
    ...
    if (mo->z - mo->momz > mo->floorz) { // Spawn splashes, etc.
      P_HitFloor(mo);
    }
    mo->z = mo->floorz;
    if (mo->momz < 0) {
      ...
      if (mo->player) {
        ...
        if (mo->momz < -GRAVITY * 8 && !(mo->flags2 & MF2_FLY)) { // squat down
...
void P_BlasterMobjThinker(mobj_t *mobj) {
  ...
  if (mobj->momx || mobj->momy || (mobj->z != mobj->floorz) || mobj->momz) {
    ...
    for (i = 0; i < 8; i++) {
      if (changexy) {
        if (!P_TryMove(mobj, mobj->x + xfrac,
                       mobj->y + yfrac)) { // Blocked move
          P_ExplodeMissile(mobj);
          return;
        }
      }
      mobj->z += zfrac;
      if (mobj->z <= mobj->floorz) { // Hit the floor
        mobj->z = mobj->floorz;
        P_HitFloor(mobj);
        P_ExplodeMissile(mobj);
        return;
      }
      if (mobj->z + mobj->height > mobj->ceilingz) { // Hit the ceiling
  ...
  if (mobj->tics != -1) {
    mobj->tics--;
    while (!mobj->tics) {
      if (!P_SetMobjState(mobj, mobj->state->nextstate)) { // mobj was removed
        return;
      }
    }
  }
...
void P_MobjThinker(mobj_t *mobj) {
  ...
  */
  ...
  if (mobj->momx || mobj->momy || (mobj->flags & MF_SKULLFLY)) {
    P_XYMovement(mobj);
    if (mobj->thinker.function == (think_t)-1) { // mobj was removed
      return;
    }
  } else if (mobj->flags2 &
             MF2_BLASTED) { // Reset to not blasted when momentums are gone
    ResetBlasted(mobj);
  }
  if (mobj->flags2 &
      MF2_FLOATBOB) { // Floating item bobbing motion (special1 is height)
    mobj->z = mobj->floorz + mobj->special1.i +
      ...
      if (!(onmo = P_CheckOnmobj(mobj))) {
        ...
        /* Landing on another player, and mimicking his movements
                                        if(mobj->player && onmo->player)
                                        {
                                                ...
                                                {
                                                        mobj->z +=
           onmo->floorz-onmo->z; if(onmo->player)
                                                        {
                                                                onmo->player->viewheight
           -= onmo->floorz-onmo->z; onmo->player->deltaviewheight = (VIEWHEIGHT-
...
void P_RemoveMobj(mobj_t *mobj) {
  ...
  if (mobj->tid) { // Remove from TID list
...
void P_SpawnPlayer(mapthing_t *mthing) {
  ...
  if (mthing->type - 1 >= maxplayers ||
      !playeringame[mthing->type - 1]) { // Not playing
    return;
  }
  ...
  if (p->class == PCLASS_FIGHTER && (mthing->type == 1 || mthing->type == 3)) {
    ...
    if (mthing->type == 1) {
      mobj->flags |= 2 << MF_TRANSSHIFT;
             ...
             1) { // Set color translation bits for player sprites
  ...
  P_SetupPsprites(p);
  if (deathmatch) { // Give all keys in death match mode
    p->keys = 2047;
  }
}
...
void P_SpawnMapThing(mapthing_t *mthing) {
  ...
  mobj_t *mobj;
  fixed_t x, y, z;
  static unsigned int classFlags[] = {MTF_FIGHTER, MTF_CLERIC, MTF_MAGE};
  ...
  if (mthing->type == PO_ANCHOR_TYPE) { // Polyobj Anchor Pt.
    return;
  } else if (mthing->type == PO_SPAWN_TYPE ||
             mthing->type == PO_SPAWNCRUSH_TYPE) { // Polyobj Anchor Pt.
    po_NumPolyobjs++;
    return;
  }
  ...
  if (netgame == false) { // Single player
    if ((mthing->options & classFlags[PlayerClass[0]]) ==
        0) { // Not for current class
      return;
    }
  } else if (deathmatch == false) { // Cooperative
    spawnMask = 0;
  ...
  if (i == NUMMOBJTYPES) { // Can't find thing type
...
void P_CreateTIDList(void) {
  ...
  i = 0;
  for (t = thinkercap.next; t != &thinkercap;
       t = t->next) {                   // Search all current thinkers
    if (t->function != P_MobjThinker) { // Not a mobj thinker
      continue;
    }
    mobj = (mobj_t *)t;
    if (mobj->tid != 0) { // Add to list
      if (i == MAX_TID_COUNT) {
...
void P_InsertMobjIntoTIDList(mobj_t *mobj, int tid) {
  ...
  for (i = 0; TIDList[i] != 0; i++) {
    if (TIDList[i] == -1) { // Found empty slot
      index = i;
      break;
    }
  }
  if (index == -1) { // Append required
    if (i == MAX_TID_COUNT) {
...
int P_HitFloor(mobj_t *thing) {
  ...
  if (thing->floorz !=
      thing->subsector->sector->floorheight) { // don't splash if landing on the
                                               // edge above water/lava/etc....
    return (FLOOR_SOLID);
  }
  ...
  case MT_LEAF1:
  case MT_LEAF2:
    //              case MT_BLOOD:                  // I set these to low mass
    //              -- pm case MT_BLOODSPLATTER:
  case MT_SPLASH:
  case MT_SLUDGECHUNK:
    return (FLOOR_SOLID);
  default:
...
mobj_t *P_SpawnMissileAngleSpeed(mobj_t *source, mobjtype_t type, angle_t angle,
                                 fixed_t momz, fixed_t speed) {
  fixed_t z;
  mobj_t *mo;
...
mobj_t *P_SpawnPlayerMissile(mobj_t *source, mobjtype_t type) {
  ...
  MissileMobj->momx =
      FixedMul(MissileMobj->info->speed, finecosine[an >> ANGLETOFINESHIFT]);
  MissileMobj->momy =
      FixedMul(MissileMobj->info->speed, finesine[an >> ANGLETOFINESHIFT]);
  MissileMobj->momz = FixedMul(MissileMobj->info->speed, slope);
  if (MissileMobj->type == MT_MWAND_MISSILE ||
      MissileMobj->type ==
          MT_CFLAME_MISSILE) { // Ultra-fast ripper spawning missile
  ...
  if (!P_TryMove(MissileMobj, MissileMobj->x,
                 MissileMobj->y)) { // Exploded immediately
