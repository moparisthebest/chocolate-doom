...
static MenuItem_t ClassItems[] = {{ITT_EFUNC, "FIGHTER", SCClass, 0, MENU_NONE},
                                  {ITT_EFUNC, "CLERIC", SCClass, 1, MENU_NONE},
                                  {ITT_EFUNC, "MAGE", SCClass, 2, MENU_NONE}};

static Menu_t ClassMenu = {66, 66, DrawClassMenu, 3, ClassItems, 0, MENU_MAIN};
...
static Menu_t SkillMenu = {120,        44, DrawSkillMenu, 5,
                           SkillItems, 2,  MENU_CLASS};
...
static Menu_t OptionsMenu = {88,           30, DrawOptionsMenu, 5,
                             OptionsItems, 0,  MENU_MAIN};
...
static Menu_t Options2Menu = {
    90, 20, DrawOptions2Menu, 6, Options2Items, 0, MENU_OPTIONS};

static Menu_t *Menus[] = {&MainMenu,     &ClassMenu, &SkillMenu, &OptionsMenu,
                          &Options2Menu, &FilesMenu, &LoadMenu,  &SaveMenu};

static const char *GammaText[] = {TXT_GAMMA_LEVEL_OFF, TXT_GAMMA_LEVEL_1,
...
const char *QuitEndMsg[] = {"ARE YOU SURE YOU WANT TO QUIT?",
...
void MN_Drawer(void) {
  ...
  if (MenuActive == false) {
    if (askforquit) {
      ...
      if (typeofask == 3) {
        MN_DrTextA(SlotText[quicksave - 1],
                   160 - MN_TextAWidth(SlotText[quicksave - 1]) / 2, 90);
        MN_DrTextA("?", 160 + MN_TextAWidth(SlotText[quicksave - 1]) / 2, 90);
      }
      if (typeofask == 4) {
        MN_DrTextA(SlotText[quickload - 1],
                   160 - MN_TextAWidth(SlotText[quickload - 1]) / 2, 90);
        MN_DrTextA("?", 160 + MN_TextAWidth(SlotText[quicksave - 1]) / 2, 90);
      }
...
static void DrawMainMenu(void) {
  ...
  V_DrawPatch(37, 80,
              W_CacheLumpNum(MauloBaseLump + (frame + 2) % 7, PU_CACHE));
...
static void DrawSkillMenu(void) { MN_DrTextB("CHOOSE SKILL LEVEL:", 74, 16); }
...
static boolean SCNetCheck(int option) {
  ...
  case 1: // new game
    ...
    break;
  case 2: // load game
    P_SetMessage(&players[consoleplayer], "YOU CAN'T LOAD A GAME IN NETPLAY!",
                 true);
    break;
  case 3: // end game
    P_SetMessage(&players[consoleplayer], "YOU CAN'T END A GAME IN NETPLAY!",
                 true);
    break;
  }
  MenuActive = false;
...
static void SCLoadGame(int option) {
  ...
  if (!SlotStatus[option]) { // Don't try to load from an empty slot
    return;
  }
  G_LoadGame(option);
  MN_DeactivateMenu();
...
boolean MN_Responder(event_t *event) {
  ...
  if (testcontrols) {
    if (event->type == ev_quit ||
        (event->type == ev_keydown && (event->data1 == key_menu_activate ||
  ...
  if ((ravpic && key == KEY_F1) || (key != 0 && key == key_menu_screenshot)) {
    G_ScreenShot();
    return (true);
  }
    ...
    } else if (key == key_menu_qsave) // F6 (quicksave)
    {
      if (gamestate == GS_LEVEL && !demoplayback) {
        if (!quicksave || quicksave == -1) {
          ...
          quicksave = -1;
          P_SetMessage(&players[consoleplayer], "CHOOSE A QUICKSAVE SLOT",
                       true);
        } else {
          askforquit = true;
          typeofask = 3;
    ...
    } else if (key == key_menu_qload) // F9 (quickload)
    {
      if (SCNetCheck(2)) {
        if (!quickload || quickload == -1) {
          ...
          slottextloaded = false; // reload the slot text
          quickload = -1;
          P_SetMessage(&players[consoleplayer], "CHOOSE A QUICKLOAD SLOT",
                       true);
        } else {
          askforquit = true;
          if (!netgame && !demoplayback) {
  ...
  if (!MenuActive) {
    if (key == key_menu_activate || gamestate == GS_DEMOSCREEN ||
        demoplayback) {
      MN_ActivateMenu();
      return (true);
    }
  ...
  if (!FileMenuKeySteal) {
    ...
    if (slotptr < SLOTTEXTLEN && key != KEY_BACKSPACE) {
      ...
      if (isdigit(charTyped) || charTyped == ' ' || charTyped == ',' ||
...
void MN_DrawInfo(void) {
  ...
  memcpy(I_VideoBuffer,
         (byte *)W_CacheLumpNum(W_GetNumForName("TITLE") + InfoType, PU_CACHE),
...
static void DrawSlider(Menu_t *menu, int item, int width, int slot) {
  ...
  for (x2 = x, count = width; count--; x2 += 8) {
    V_DrawPatch(x2, y,
                W_CacheLumpName(count & 1 ? "M_SLDMD1" : "M_SLDMD2", PU_CACHE));
  }
