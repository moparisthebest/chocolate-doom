...
const char *TextKeyMessages[] = {
    TXT_KEY_STEEL,   TXT_KEY_CAVE,    TXT_KEY_AXE,    TXT_KEY_FIRE,
    TXT_KEY_EMERALD, TXT_KEY_DUNGEON, TXT_KEY_SILVER, TXT_KEY_RUSTED,
    TXT_KEY_HORN,    TXT_KEY_SWAMP,   TXT_KEY_CASTLE};
...
boolean P_GiveMana(player_t *player, manatype_t mana, int count) {
  ...
  if (gameskill == sk_baby ||
      gameskill == sk_nightmare) { // extra mana in baby mode and nightmare mode
    count += count >> 1;
  }
...
static void TryPickupWeapon(player_t *player, pclass_t weaponClass,
  ...
  remove = true;
  if (player->class !=
      weaponClass) {              // Wrong class, but try to pick up for mana
    if (netgame && !deathmatch) { // Can't pick up weapons for other classes in
                                  // coop netplay
      return;
    }
  ...
  } else if (netgame && !deathmatch) { // Cooperative net-game
    ...
    remove = false;
  } else { // Deathmatch or single player game
    if (weaponType == WP_SECOND) {
    ...
    if (player->weaponowned[weaponType]) {
      ...
      player->weaponowned[weaponType] = true;
      if (weaponType >
          player->readyweapon) { // Only switch to more powerful weapons
        player->pendingweapon = weaponType;
      }
    }
    if (!(gaveWeapon ||
          gaveMana)) { // Player didn't need the weapon or any mana
      return;
    }
  }
  ...
  if (weapon->special) {
    P_ExecuteLineSpecial(weapon->special, weapon->args, NULL, 0, player->mo);
    weapon->special = 0;
  }
...
static void TryPickupWeaponPiece(player_t *player, pclass_t matchClass,
  ...
  boolean gaveWeapon;
  int gaveMana;
  static const char *fourthWeaponText[] = {TXT_WEAPON_F4, TXT_WEAPON_C4,
                                           TXT_WEAPON_M4};
  static const char *weaponPieceText[] = {
      TXT_QUIETUS_PIECE, TXT_WRAITHVERGE_PIECE, TXT_BLOODSCOURGE_PIECE};
  ...
  checkAssembled = true;
  gaveWeapon = false;
  if (player->class != matchClass) { // Wrong class, but try to pick up for mana
    if (netgame &&
        !deathmatch) { // Can't pick up wrong-class weapons in coop netplay
      return;
    }
    checkAssembled = false;
    gaveMana = P_GiveMana(player, MANA_1, 20) + P_GiveMana(player, MANA_2, 20);
    if (!gaveMana) { // Didn't need the mana, so don't pick it up
      return;
    }
  } else if (netgame && !deathmatch) { // Cooperative net-game
    if (player->pieces & pieceValue) { // Already has the piece
      return;
    }
    ...
    gaveMana = P_GiveMana(player, MANA_1, 20) + P_GiveMana(player, MANA_2, 20);
    if (player->pieces &
        pieceValue) {  // Already has the piece, check if mana needed
      if (!gaveMana) { // Didn't need the mana, so don't pick it up
        return;
      }
      checkAssembled = false;
...
boolean P_GivePower(player_t *player, powertype_t power) {
  if (power == pw_invulnerability) {
    if (player->powers[power] > BLINKTHRESHOLD) { // Already have it
      return (false);
    }
...
static void TryPickupArtifact(player_t *player, artitype_t artifactType,
  ...
  if (P_GiveArtifact(player, artifactType, artifact)) {
    if (artifact->special) {
      P_ExecuteLineSpecial(artifact->special, artifact->args, NULL, 0, NULL);
      artifact->special = 0;
    }
    player->bonuscount += BONUSADD;
    if (artifactType < arti_firstpuzzitem) {
      ...
      if (!netgame ||
          deathmatch) { // Remove puzzle items if not cooperative netplay
...
boolean P_GiveArtifact(player_t *player, artitype_t arti, mobj_t *mo) {
  ...
  if (i == player->inventorySlotNum) {
    ...
    if (arti >= arti_firstpuzzitem && netgame &&
        !deathmatch) { // Can't carry more than 1 puzzle item in coop netplay
      return false;
    }
    if (player->inventory[i].count >=
        25) { // Player already has 25 of this item
      return false;
    }
...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher) {
  ...
  if (delta > toucher->height || delta < -32 * FRACUNIT) { // Out of reach
    return;
  }
  if (toucher->health <= 0) { // Toucher is dead
    return;
  }
  ...
  case SPR_KEYB:
    ...
    if (special->special) {
      P_ExecuteLineSpecial(special->special, special->args, NULL, 0, toucher);
      special->special = 0;
    }

    if (!netgame) { // Only remove keys in single player game
      break;
    }
    player->bonuscount += BONUSADD;
    if (player == &players[consoleplayer]) {
  ...
  case SPR_WMCS: // Frost Shards
    TryPickupWeapon(player, PCLASS_MAGE, WP_SECOND, special, TXT_WEAPON_M2);
    return;
  ...
  case SPR_WFAX: // Timon's Axe
    TryPickupWeapon(player, PCLASS_FIGHTER, WP_SECOND, special, TXT_WEAPON_F2);
    return;
  case SPR_WFHM: // Hammer of Retribution
    TryPickupWeapon(player, PCLASS_FIGHTER, WP_THIRD, special, TXT_WEAPON_F3);
    return;

    // 2nd and 3rd Cleric Weapons
  case SPR_WCSS: // Serpent Staff
    TryPickupWeapon(player, PCLASS_CLERIC, WP_SECOND, special, TXT_WEAPON_C2);
    return;
  case SPR_WCFM: // Firestorm
    TryPickupWeapon(player, PCLASS_CLERIC, WP_THIRD, special, TXT_WEAPON_C3);
    return;
...
void P_KillMobj(mobj_t *source, mobj_t *target) {
  ...
  if ((target->flags & MF_COUNTKILL || target->type == MT_ZBELL) &&
      target->special) { // Initiate monster death actions
  ...
  if (source && source->player) { // Check for frag changes
    if (target->player) {
      if (target == source) { // Self-frag
        target->player->frags[target->player - players]--;
        if (cmdfrag && netgame &&
            source->player ==
                &players[consoleplayer]) { // Send out a frag count packet
  ...
  if (target->player) { // Player death
    ...
    if (target->flags2 & MF2_FIREDAMAGE) { // Player flame death
    ...
    if (target->flags2 & MF2_ICEDAMAGE) {     // Player ice death
  ...
  if (target->health < -(target->info->spawnhealth >> 1) &&
      target->info->xdeathstate) { // Extreme death
...
boolean P_MorphPlayer(player_t *player) {
  ...
  if (player->powers[pw_invulnerability]) { // Immune when invulnerable
    return (false);
  }
  if (player->morphTics) { // Player is already a beast
    return false;
  }
  pmo = player->mo;
  x = pmo->x;
...
void P_AutoUseHealth(player_t *player, int saveHealth) {
  ...
  if ((gameskill == sk_baby) &&
      (normalCount * 25 >= saveHealth)) { // Use quartz flasks
    count = (saveHealth + 24) / 25;
    for (i = 0; i < count; i++) {
      player->health += 25;
      P_PlayerRemoveArtifact(player, normalSlot);
    }
  } else if (superCount * 100 >= saveHealth) { // Use mystic urns
    ...
    for (i = 0; i < count; i++) {
      ...
      P_PlayerRemoveArtifact(player, superSlot);
             ...
             (superCount * 100 + normalCount * 25 >=
              saveHealth)) { // Use mystic urns and quartz flasks
    count = (saveHealth + 24) / 25;
    saveHealth -= count * 25;
    for (i = 0; i < count; i++) {
      player->health += 25;
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source,
  ...
  if ((target->flags2 & MF2_INVULNERABLE) &&
      damage < 10000) { // mobj is invulnerable
    if (target->player)
  ...
  if (inflictor) {
    ...
    case MT_TELOTHER_FX5:
      ...
      return;
    case MT_MINOTAUR:
      if (inflictor->flags & MF_SKULLFLY) { // Slam only when in charge mode
    ...
    case MT_POISONCLOUD:
      if (target->player) {
        if (target->player->poisoncount < 4) {
          P_PoisonDamage(target->player, source, 15 + (P_Random() & 15),
                         false); // Don't play painsound
          P_PoisonPlayer(target->player, source, 50);
          S_StartSound(target, SFX_PLAYER_POISONCOUGH);
        }
        return;
      } else if (!(target->flags &
                   MF_COUNTKILL)) { // only damage monsters/players with the
                                    // poison cloud
        return;
      }
      break;
  ...
  if (player) {
    savedPercent =
        AutoArmorSave[player->class] + player->armorpoints[ARMOR_ARMOR] +
        player->armorpoints[ARMOR_SHIELD] + player->armorpoints[ARMOR_HELMET] +
        player->armorpoints[ARMOR_AMULET];
    if (savedPercent) { // armor absorbed some damage
      ...
      for (i = 0; i < NUMARMOR; i++) {
        if (player->armorpoints[i]) {
          player->armorpoints[i] -= FixedDiv(
              FixedMul(damage << FRACBITS, ArmorIncrement[player->class][i]),
              300 * FRACUNIT);
      ...
      saved =
          FixedDiv(FixedMul(damage << FRACBITS, savedPercent), 100 * FRACUNIT);
    ...
    if (damage >= player->health && ((gameskill == sk_baby) || deathmatch) &&
        !player->morphTics) { // Try to use some inventory health
  ...
  if (target->health <= 0) { // Death
    if (inflictor) { // check for special fire damage or ice damage deaths
      if (inflictor->flags2 & MF2_FIREDAMAGE) {
        if (player && !player->morphTics) { // Check for flame death
    ...
    if (source &&
        (source->type == MT_MINOTAUR)) { // Minotaur's kills go to his master
...
void P_PoisonDamage(player_t *player, mobj_t *source, int damage,
  ...
  if (target->health <= 0) { // Death
    ...
    if (inflictor && !player->morphTics) { // Check for flame death
