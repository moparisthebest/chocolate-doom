...
void SB_SetClassData(void) {
  ...
  PatchWEAPONSLOT =
      W_CacheLumpNum(W_GetNumForName("wpslot0") + class, PU_STATIC);
  PatchWEAPONFULL =
      W_CacheLumpNum(W_GetNumForName("wpfull0") + class, PU_STATIC);
  PatchPIECE1 = W_CacheLumpNum(W_GetNumForName("wpiecef1") + class, PU_STATIC);
  PatchPIECE2 = W_CacheLumpNum(W_GetNumForName("wpiecef2") + class, PU_STATIC);
  PatchPIECE3 = W_CacheLumpNum(W_GetNumForName("wpiecef3") + class, PU_STATIC);
  PatchCHAIN = W_CacheLumpNum(W_GetNumForName("chain") + class, PU_STATIC);
  if (!netgame) { // single player game uses red life gem (the second gem)
    PatchLIFEGEM = W_CacheLumpNum(
        W_GetNumForName("lifegem") + maxplayers * class + 1, PU_STATIC);
  } else {
...
static void DrawAnimatedIcons(void) {
    ...
    if (CPlayer->powers[pw_flight] > BLINKTHRESHOLD ||
      ...
      if (CPlayer->mo->flags2 & MF2_FLY) {
        if (hitCenterFrame && (frame != 15 && frame != 0)) {
          V_DrawPatch(20, 19, W_CacheLumpNum(SpinFlylump + 15, PU_CACHE));
        } else {
          V_DrawPatch(20, 19, W_CacheLumpNum(SpinFlylump + frame, PU_CACHE));
  ...
  if (CPlayer->powers[pw_speed]) {
    if (CPlayer->powers[pw_speed] > BLINKTHRESHOLD ||
      ...
      frame = (leveltime / 3) & 15;
      V_DrawPatch(60, 19, W_CacheLumpNum(SpinSpeedLump + frame, PU_CACHE));
    }
    BorderTopRefresh = true;
    UpdateState |= I_MESSAGES;
  }
  ...
  if (CPlayer->powers[pw_invulnerability]) {
    if (CPlayer->powers[pw_invulnerability] > BLINKTHRESHOLD ||
      ...
      frame = (leveltime / 3) & 15;
      V_DrawPatch(260, 19, W_CacheLumpNum(SpinDefenseLump + frame, PU_CACHE));
    }
    BorderTopRefresh = true;
    UpdateState |= I_MESSAGES;
  }
  ...
  if (CPlayer->powers[pw_minotaur]) {
    if (CPlayer->powers[pw_minotaur] > BLINKTHRESHOLD ||
      ...
      frame = (leveltime / 3) & 15;
      V_DrawPatch(300, 19, W_CacheLumpNum(SpinMinotaurLump + frame, PU_CACHE));
    }
    BorderTopRefresh = true;
    UpdateState |= I_MESSAGES;
  }
...
void SB_PaletteFlash(boolean forceChange) {
  ...
  if (gamestate == GS_LEVEL) {
    ...
    } else if (CPlayer->mo->flags2 & MF2_ICEDAMAGE) { // Frozen player
      palette = STARTICEPAL;
    } else {
      palette = 0;
    }
...
void DrawMainBar(void) {
  ...
  if (ArtifactFlash) {
    ...
    V_DrawPatch(148, 164,
                W_CacheLumpNum(W_GetNumForName("useartia") + ArtifactFlash - 1,
                               PU_CACHE));
    ArtifactFlash--;
  ...
  if (oldweapon != CPlayer->readyweapon || manaPatch1 || manaPatch2 ||
      manaVialPatch1) { // Update mana graphics based upon mana count/weapon
                        // type
  ...
  // Armor
  temp = AutoArmorSave[CPlayer->class] + CPlayer->armorpoints[ARMOR_ARMOR] +
...
void DrawInventoryBar(void) {
  ...
  for (i = 0; i < 7; i++) {
    ...
    if (CPlayer->inventorySlotNum > x + i &&
      ...
      V_DrawPatch(
          50 + i * 31, 163,
          W_CacheLumpName(patcharti[CPlayer->inventory[x + i].type], PU_CACHE));
    ...
    for (i = 0; i < 7; i++) {
      ...
      if (CPlayer->inventorySlotNum > x + i &&
        ...
        V_DrawPatch(49 + i * 31, 167,
                    W_CacheLumpName(patcharti[CPlayer->inventory[x + i].type],
                                    PU_CACHE));
        if (CPlayer->inventory[x + i].count > 1) {
...
boolean SB_Responder(event_t *event) {
  if (event->type == ev_keydown) {
    if (HandleCheats(event->data1)) { // Need to eat the key
      return (true);
    }
  }
  return (false);
...
static boolean HandleCheats(byte key) {
  ...
  if (gameskill == sk_nightmare) { // Can't cheat in nightmare mode
    return (false);
  } else if (netgame) { // change CD track is the only cheat available in
                        // deathmatch
    eat = false;
    if (cdmusic) {
...
static void CheatWarpFunc(player_t *player, Cheat_t *cheat) {
  ...
  tens = args[0] - '0';
  ones = args[1] - '0';
  if (tens < 0 || tens > 9 || ones < 0 || ones > 9) { // Bad map
  ...
  if (map == gamemap) { // Don't try to teleport to current map
    P_SetMessage(player, TXT_CHEATBADINPUT, true);
    return;
  }
...
static void CheatClassFunc2(player_t *player, Cheat_t *cheat) {
  ...
  if (player->morphTics) { // don't change class if the player is morphed
    return;
  }
  class = args[0] - '0';
  if (class > 2 || class < 0) {
...
static void CheatDebugFunc(player_t *player, Cheat_t *cheat) {
  char textBuffer[50];
  M_snprintf(textBuffer, sizeof(textBuffer), "MAP %d (%d)  X:%5d  Y:%5d  Z:%5d",
...
static void CheatScriptFunc3(player_t *player, Cheat_t *cheat) {
  ...
  if (P_StartACS(script, 0, script_args, player->mo, NULL, 0)) {
    M_snprintf(textBuffer, sizeof(textBuffer), "RUNNING SCRIPT %.2d", script);
    P_SetMessage(player, textBuffer, true);
  }
}
