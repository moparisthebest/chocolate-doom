...
cliprange_t solidsegs[MAXSEGS],
    *newend; // newend is one past the last valid seg
...
void R_ClipSolidWallSegment(int first, int last) {
  ...
  if (first < start->first) {
    if (last < start->first - 1) { // post is entirely visible (above start), so
                                   // insert a new clippost
  ...
  while (last >= (next + 1)->first - 1) {
    ...
    next++;
    if (last <= next->last) {   // bottom is contained in next
...
void R_ClipPassWallSegment(int first, int last) {
  ...
  if (first < start->first) {
    if (last < start->first - 1) { // post is entirely visible (above start)
      R_StoreWallRange(first, last);
      return;
    }
...
boolean R_CheckBBox(fixed_t *bspcoord) {
  ...
  // find the first clippost that touches the source post (adjacent pixels are
  // touching)
  angle1 = (angle1 + ANG90) >> ANGLETOFINESHIFT;
  angle2 = (angle2 + ANG90) >> ANGLETOFINESHIFT;
