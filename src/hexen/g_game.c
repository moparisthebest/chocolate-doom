...
fixed_t forwardmove[NUMCLASSES][2] = {
    {0x1D, 0x3C}, {0x19, 0x32}, {0x16, 0x2E}, {0x18, 0x31}};

fixed_t sidemove[NUMCLASSES][2] = {
    {0x1B, 0x3B}, {0x18, 0x28}, {0x15, 0x25}, {0x17, 0x27}};
...
void G_BuildTiccmd(ticcmd_t *cmd, int maketic) {
  ...
  if (gamekeydown[key_useartifact]) {
    if (gamekeydown[key_speed] && artiskip) {
      if (players[consoleplayer].inventory[inv_ptr].type !=
          arti_none) { // Skip an artifact
  ...
  if (gamestate == GS_LEVEL && players[consoleplayer].morphTics == 0 &&
      next_weapon != 0) {
    int start_i;
  ...
  if (players[consoleplayer].powers[pw_speed] &&
      !players[consoleplayer]
           .morphTics) { // Adjust for a player with a speed artifact
    forward = (3 * forward) >> 1;
    side = (3 * side) >> 1;
  }
...
boolean G_Responder(event_t *ev) {
  ...
  if (ev->type == ev_keyup &&
      ev->data1 ==
          key_useartifact) { // flag to denote that it's okay to use an artifact
    if (!inventory) {
  ...
  if (gamestate == GS_LEVEL && ev->type == ev_keydown && ev->data1 == key_spy &&
      !deathmatch) { // Cycle the display player
    do {
      displayplayer++;
  ...
  if (gamestate == GS_LEVEL) {
    ...
    if (AM_Responder(ev)) { // Automap ate the event
      return (true);
    }
  }
...
void G_PlayerExitMap(int playerNumber) {
  ...
  if (deathmatch) {
    ...
    if (P_GetMapCluster(gamemap) !=
        P_GetMapCluster(LeaveMap)) { // Entering new cluster
      // Strip all keys
      player->keys = 0;
...
void G_DoReborn(int playernum) {
  ...
  if (!netgame) {
    if (SV_RebornSlotAvailable()) { // Use the reborn code if the slot is
                                    // available
      gameaction = ga_singlereborn;
    } else { // Start a new game if there's no reborn info
      gameaction = ga_newgame;
    }
  } else {                                // Net-game
    players[playernum].mo->player = NULL; // Dissassociate the corpse

    if (deathmatch) { // Spawn at random spot if in death match
    ...
    if (G_CheckSpot(
            playernum,
            &playerstarts[RebornPosition][playernum])) { // Appropriate player
                                                         // start spot is open
      ...
      for (i = 0; i < maxplayers; i++) {
        if (G_CheckSpot(
                playernum,
                &playerstarts[RebornPosition][i])) { // Found an open start spot
    ...
    if (foundSpot == false) { // Player's going to be inside something
      P_SpawnPlayer(&playerstarts[RebornPosition][playernum]);
    }
    ...
    if (bestWeapon) { // Bring up the best weapon
...
void G_WorldDone(void) { gameaction = ga_worlddone; }
...
void G_DoLoadGame(void) {
  ...
  if (!netgame) { // Copy the base slot to the reborn slot
    SV_UpdateRebornSlot();
  }
  SB_SetClassData();
}
...
void G_ReadDemoTiccmd(ticcmd_t *cmd) {
  if (*demo_p == DEMOMARKER) { // end of demo data stream
    G_CheckDemoStatus();
    return;
  }
...
void G_RecordDemo(skill_t skill, int numplayers, int episode, int map,
  ...
  longtics =
      D_NonVanillaRecord(M_ParmExists("-longtics"), "vvHeretic longtics demo");
...
void G_DoPlayDemo(void) {
  ...
  // vvHexen.
  if (D_NonVanillaPlayback((*demo_p & DEMOHEADER_LONGTICS) != 0, lumpnum,
                           "vvHeretic longtics demo")) {
    longtics = true;
  }
  if (D_NonVanillaPlayback((*demo_p & DEMOHEADER_RESPAWN) != 0, lumpnum,
                           "vvHeretic -respawn header flag")) {
    respawnparm = true;
  }
  if (D_NonVanillaPlayback((*demo_p & DEMOHEADER_NOMONSTERS) != 0, lumpnum,
