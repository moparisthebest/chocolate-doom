...
void SV_LoadGame(int slot) {
  ...
  if (strncmp(version_text, HXS_VERSION_TEXT, HXS_VERSION_TEXT_LENGTH) !=
      0) { // Bad version
    return;
  }
...
void SV_ClearRebornSlot(void) { ClearSaveSlot(REBORN_SLOT); }
...
void SV_MapTeleport(int map, int position) {
  ...
  if (!deathmatch) {
    if (P_GetMapCluster(gamemap) ==
        P_GetMapCluster(
            map)) { // Same cluster - save map without saving player mobjs
      SV_SaveMap(false);
    } else { // Entering new cluster - clear base slot
      ClearSaveSlot(BASE_SLOT);
    }
  }
  ...
  gamemap = map;
  M_snprintf(fileName, sizeof(fileName), "%shex6%02d.hxs", SavePath, gamemap);
  if (!deathmatch && ExistingFile(fileName)) { // Unarchive map
    SV_LoadMap();
  } else { // New map
  ...
  for (i = 0; i < maxplayers; i++) {
    ...
    if (netgame) {
      if (players[i].playerstate ==
          PST_DEAD) { // In a network game, force all players to be alive
        players[i].playerstate = PST_REBORN;
      }
      if (!deathmatch) { // Cooperative net-play, retain keys and weapons
    ...
    if (deathmatch) {
      ...
      mobj = P_SpawnMobj(playerstarts[0][i].x << 16, playerstarts[0][i].y << 16,
                         0, MT_PLAYER_FIGHTER);
      players[i].mo = mobj;
    ...
    if (playerWasReborn && netgame &&
        !deathmatch) { // Restore keys and weapons when reborn in co-op
    ...
    if (targetPlayerMobj == NULL) { // The poor sap
...
int SV_GetRebornSlot(void) { return (REBORN_SLOT); }
...
void SV_InitBaseSlot(void) { ClearSaveSlot(BASE_SLOT); }
...
static void SetMobjArchiveNums(void) {
  ...
  for (thinker = thinkercap.next; thinker != &thinkercap;
    ...
    if (thinker->function == P_MobjThinker) {
      mobj = (mobj_t *)thinker;
      if (mobj->player && !SavingPlayers) { // Skipping player mobjs
        continue;
      }
...
static void ArchiveMobjs(void) {
  ...
  for (thinker = thinkercap.next; thinker != &thinkercap;
       thinker = thinker->next) {
    if (thinker->function != P_MobjThinker) { // Not a mobj thinker
      continue;
    }
    if (((mobj_t *)thinker)->player &&
        !SavingPlayers) { // Skipping player mobjs
      continue;
    }
    count++;
...
static void UnarchiveMobjs(void) {
  ...
  TargetPlayerAddrs =
      Z_Malloc(MAX_TARGET_PLAYERS * sizeof(mobj_t **), PU_STATIC, NULL);
  TargetPlayerCount = 0;
...
static thinkInfo_t ThinkerInfo[] = {
    {TC_MOVE_FLOOR, T_MoveFloor, StreamOut_floormove_t, StreamIn_floormove_t,
     RestoreSSThinker, sizeof(floormove_t)},
    {TC_PLAT_RAISE, T_PlatRaise, StreamOut_plat_t, StreamIn_plat_t,
     RestorePlatRaise, sizeof(plat_t)},
    {TC_MOVE_CEILING, T_MoveCeiling, StreamOut_ceiling_t, StreamIn_ceiling_t,
     RestoreMoveCeiling, sizeof(ceiling_t)},
    {TC_LIGHT, T_Light, StreamOut_light_t, StreamIn_light_t, NULL,
     sizeof(light_t)},
    {TC_VERTICAL_DOOR, T_VerticalDoor, StreamOut_vldoor_t, StreamIn_vldoor_t,
     RestoreSSThinker, sizeof(vldoor_t)},
    {TC_PHASE, T_Phase, StreamOut_phase_t, StreamIn_phase_t, NULL,
     sizeof(phase_t)},
    {TC_INTERPRET_ACS, T_InterpretACS, StreamOut_acs_t, StreamIn_acs_t, NULL,
     sizeof(acs_t)},
    {TC_ROTATE_POLY, T_RotatePoly, StreamOut_polyevent_t, StreamIn_polyevent_t,
     NULL, sizeof(polyevent_t)},
    {TC_BUILD_PILLAR, T_BuildPillar, StreamOut_pillar_t, StreamIn_pillar_t,
     RestoreSSThinker, sizeof(pillar_t)},
    {TC_MOVE_POLY, T_MovePoly, StreamOut_polyevent_t, StreamIn_polyevent_t,
     NULL, sizeof(polyevent_t)},
    {TC_POLY_DOOR, T_PolyDoor, StreamOut_polydoor_t, StreamIn_polydoor_t, NULL,
     sizeof(polydoor_t)},
    {TC_FLOOR_WAGGLE, T_FloorWaggle, StreamOut_floorWaggle_t,
     StreamIn_floorWaggle_t, RestoreSSThinker, sizeof(floorWaggle_t)},
...
static void ArchiveSounds(void) {
  ...
  for (node = SequenceListHead; node; node = node->next) {
    ...
    if (i == po_NumPolyobjs) { // Sound is attached to a sector, not a polyobj
...
static void ClearSaveSlot(int slot) {
  ...
  for (i = 0; i < MAX_MAPS; i++) {
    M_snprintf(fileName, sizeof(fileName), "%shex%d%02d.hxs", SavePath, slot,
               i);
    remove(fileName);
  }
...
static void CopySaveSlot(int sourceSlot, int destSlot) {
  ...
  for (i = 0; i < MAX_MAPS; i++) {
    M_snprintf(sourceName, sizeof(sourceName), "%shex%d%02d.hxs", SavePath,
               sourceSlot, i);
    if (ExistingFile(sourceName)) {
      M_snprintf(destName, sizeof(destName), "%shex%d%02d.hxs", SavePath,
                 destSlot, i);
      CopyFile(sourceName, destName);
    }
  }
  M_snprintf(sourceName, sizeof(sourceName), "%shex%d.hxs", SavePath,
             sourceSlot);
  if (ExistingFile(sourceName)) {
...
static void SV_OpenWrite(char *fileName) { SavingFP = fopen(fileName, "wb"); }
...
static void *SV_ReadPtr(void) { return (void *)(intptr_t)SV_ReadLong(); }
...
static void SV_WriteByte(byte val) { fwrite(&val, sizeof(byte), 1, SavingFP); }
