...
static const char *MapCmdNames[] = {"SKY1",           "SKY2",
                                    "DOUBLESKY",      "LIGHTNING",
                                    "FADETABLE",      "CLUSTER",
                                    "WARPTRANS",      "NEXT",
                                    "CDTRACK",        "CD_START_TRACK",
                                    "CD_END1_TRACK",  "CD_END2_TRACK",
                                    "CD_END3_TRACK",  "CD_INTERMISSION_TRACK",
                                    "CD_TITLE_TRACK", NULL};
static int MapCmdIDs[] = {
    MCMD_SKY1,         MCMD_SKY2,          MCMD_DOUBLESKY,    MCMD_LIGHTNING,
    MCMD_FADETABLE,    MCMD_CLUSTER,       MCMD_WARPTRANS,    MCMD_NEXT,
    MCMD_CDTRACK,      MCMD_CD_STARTTRACK, MCMD_CD_END1TRACK, MCMD_CD_END2TRACK,
    MCMD_CD_END3TRACK, MCMD_CD_INTERTRACK, MCMD_CD_TITLETRACK};
...
void P_LoadThings(int lump) {
  ...
  if (!deathmatch) { // Don't need to check deathmatch spots
    return;
  }
  playerCount = 0;
...
void P_SetupLevel(int episode, int map, int playermask, skill_t skill) {
  ...
  if (deathmatch) {
    for (i = 0; i < maxplayers; i++) {
      if (playeringame[i]) { // must give a player spot before deathmatchspawn
  ...
  } else { // Probably fog ... don't use fullbright sprites
...
void P_PutMapSongLump(int map, char *lumpName) {
  ...
  M_StringCopy(MapInfo[map].songLump, lumpName, sizeof(MapInfo[map].songLump));
}
...
static int QualifyMap(int map) { return (map < 1 || map > MapCount) ? 0 : map; }
