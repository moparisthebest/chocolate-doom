...
void T_RotatePoly(polyevent_t *pe) {
  ...
  if (PO_RotatePolyobj(pe->polyobj, pe->speed)) {
    ...
    if (pe->dist == -1) { // perpetual polyobj
      return;
    }
    pe->dist -= absSpeed;
    if (pe->dist <= 0) {
...
void T_PolyDoor(polydoor_t *pd) {
  ...
  case PODOOR_SLIDE:
    if (PO_MovePolyobj(pd->polyobj, pd->xSpeed, pd->ySpeed)) {
      ...
      poly = GetPolyobj(pd->polyobj);
      if (poly->crush || !pd->close) { // continue moving if the poly is a
                                       // crusher, or is opening
        return;
      } else { // open back up
  ...
  case PODOOR_SWING:
    if (PO_RotatePolyobj(pd->polyobj, pd->speed)) {
      absSpeed = abs(pd->speed);
      if (pd->dist == -1) { // perpetual polyobj
        return;
      }
      pd->dist -= absSpeed;
      if (pd->dist <= 0) {
...
boolean EV_OpenPolyDoor(line_t *line, byte *args, podoortype_t type) {
  ...
  while ((mirror = GetPolyobjMirror(polyNum)) != 0) {
    poly = GetPolyobj(mirror);
    if (poly && poly->specialdata) { // mirroring poly is already in motion
      break;
    }
    pd = Z_Malloc(sizeof(polydoor_t), PU_LEVSPEC, 0);
    memset(pd, 0, sizeof(polydoor_t));
  ...
  for (j = po->bbox[BOXBOTTOM]; j <= po->bbox[BOXTOP]; j++) {
    ...
    for (i = po->bbox[BOXLEFT]; i <= po->bbox[BOXRIGHT]; i++) {
      if (i >= 0 && i < bmapwidth && j >= 0 && j < bmapheight) {
        ...
        if (link == NULL) { // polyobj not located in the link cell
          continue;
        }
        link->polyobj = NULL;
      }
  ...
  for (j = po->bbox[BOXBOTTOM] * bmapwidth; j <= po->bbox[BOXTOP] * bmapwidth;
    ...
    for (i = po->bbox[BOXLEFT]; i <= po->bbox[BOXRIGHT]; i++) {
      if (i >= 0 && i < bmapwidth && j >= 0 && j < bmapheight * bmapwidth) {
        ...
        if (!(*link)) { // Create a new link at the current block cell
...
static void SpawnPolyobj(int index, int tag, boolean crush) {
  ...
  if (!polyobjs[index].segs) { // didn't find a polyobj through PO_LINE_START
    psIndex = 0;
    ...
    for (j = 1; j < PO_MAXPOLYSEGS; j++) {
      ...
      for (i = 0; i < numsegs; i++) {
        if (segs[i].linedef->special == PO_LINE_EXPLICIT &&
          ...
          if (!segs[i].linedef->arg2) {
            I_Error("SpawnPolyobj:  Explicit line missing order number "
      ...
      if (psIndex ==
          psIndexOld) { // Check if an explicit line order has been skipped
    ...
    if (polyobjs[index].numsegs) {
      ...
      polyobjs[index].tag = tag;
      polyobjs[index].segs =
          Z_Malloc(polyobjs[index].numsegs * sizeof(seg_t *), PU_LEVEL, 0);
...
static void TranslateToStartSpot(int tag, int originX, int originY) {
  ...
  if (!po) { // didn't match the tag with a polyobj tag
    I_Error("TranslateToStartSpot:  Unable to match polyobj tag: %d\n", tag);
  }
  if (po->segs == NULL) {
    I_Error("TranslateToStartSpot:  Anchor point located without a StartSpot "
            "point: %d\n",
            tag);
  }
  ...
  for (i = 0; i < po->numsegs; i++, tempSeg++, tempPt++) {
    ...
    if (veryTempSeg == tempSeg) { // the point hasn't been translated, yet
      (*tempSeg)->v1->x -= deltaX;
      (*tempSeg)->v1->y -= deltaY;
    }
...
void PO_Init(int lump) {
  ...
  for (i = 0; i < numthings; i++, mt++) {
    ...
    if (spawnthing.type == PO_SPAWN_TYPE ||
        spawnthing.type == PO_SPAWNCRUSH_TYPE) { // Polyobj StartSpot Pt.
  ...
  for (i = 0; i < numthings; i++, mt++) {
    ...
    spawnthing.angle = SHORT(mt->angle);
    spawnthing.type = SHORT(mt->type);
    if (spawnthing.type == PO_ANCHOR_TYPE) { // Polyobj Anchor Pt.
      TranslateToStartSpot(spawnthing.angle, spawnthing.x << FRACBITS,
