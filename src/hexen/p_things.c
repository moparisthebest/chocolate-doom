...
boolean EV_ThingProjectile(byte *args, boolean gravity) {
  ...
  if (nomonsters && (mobjinfo[moType].flags &
                     MF_COUNTKILL)) { // Don't spawn monsters if -nomonsters
    return false;
  }
  angle = (int)args[2] << 24;
...
boolean EV_ThingSpawn(byte *args, boolean fog) {
  ...
  while ((mobj = P_FindMobjFromTID(tid, &searcher)) != NULL) {
    ...
    if (P_TestMobjLocation(newMobj) == false) { // Didn't fit
      P_RemoveMobj(newMobj);
    } else {
      newMobj->angle = angle;
      if (fog == true) {
        fogMobj =
            P_SpawnMobj(mobj->x, mobj->y, mobj->z + TELEFOGHEIGHT, MT_TFOG);
...
static boolean ActivateThing(mobj_t *mobj) {
  if (mobj->flags & MF_COUNTKILL) { // Monster
