...
static struct {
  ...
  int stopSound;
    ...
    {"Platform", 0, 0},
    {"Platform", 0, 0}, // a 'heavy' platform is just a
                        // platform
    {"PlatformMetal", 0, 0},
...
void SN_UpdateActiveSequences(void) {
  ...
  if (!ActiveSequences ||
      paused) { // No sequences currently playing/game is paused
    return;
  }
  for (node = SequenceListHead; node; node = node->next) {
    if (node->delayTics) {
...
void SN_ChangeNodeData(int nodeNum, int seqOffset, int delayTics, int volume,
  ...
  if (!node) { // reach the end of the list before finding the nodeNum-th node
    return;
  }
