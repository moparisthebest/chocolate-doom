...
static const char *ClusMsgLumpNames[] = {"clus1msg", "clus2msg", "clus3msg",
                                         "clus4msg", "clus5msg"};
...
static void CheckForSkip(void) {
  ...
  if (deathmatch &&
      intertime < 140) { // wait for 4 seconds before allowing a skip
...
static void DrDeathTally(void) {
  ...
  if (intertime < TALLY_EFFECT_TICKS) {
    ...
    xStart =
        TALLY_START_XPOS - FixedMul(scale, TALLY_START_XPOS - TALLY_STOP_XPOS);
    yPos =
