...
weaponinfo_t WeaponInfo[NUMWEAPONS][NUMCLASSES] = {
    {// First Weapons
     ...
     {// Mage First Weapon - Wand
      MANA_NONE, S_MWANDUP, S_MWANDDOWN, S_MWANDREADY, S_MWANDATK_1,
      S_MWANDATK_1, S_NULL},
     {
         // Pig - Snout
         MANA_NONE,    // mana
...
static int WeaponManaUse[NUMCLASSES][NUMWEAPONS] = {
    {0, 2, 3, 14}, {0, 1, 4, 18}, {0, 3, 5, 15}, {0, 0, 0, 0}};
...
void P_SetPsprite(player_t *player, int position, statenum_t stnum) {
  ...
  do {
    ...
    if (state->misc1) {      // Set coordinates.
      psp->sx = state->misc1 << FRACBITS;
    }
    if (state->misc2) {
      psp->sy = state->misc2 << FRACBITS;
    }
    if (state->action) { // Call action routine.
...
void P_FireWeapon(player_t *player) {
  ...
  if (player->class == PCLASS_FIGHTER && player->readyweapon == WP_SECOND &&
      player->mana[MANA_1] > 0) { // Glowing axe
...
void A_Lower(player_t *player, pspdef_t *psp) {
  ...
  if (psp->sy < WEAPONBOTTOM) { // Not lowered all the way yet
    return;
  }
  if (player->playerstate ==
      PST_DEAD) { // Player is dead, so don't bring up a pending weapon
    psp->sy = WEAPONBOTTOM;
    return;
  }
...
hammerdone:
  if (player->mana[MANA_2] <
      WeaponManaUse[player->class]
                   [player->readyweapon]) { // Don't spawn a hammer if the
...
void A_FSwordFlames(mobj_t *actor) {
  ...
  for (i = 1 + (P_Random() & 3); i; i--) {
    ...
    r2 = P_Random();
    r3 = P_Random();
    P_SpawnMobj(actor->x + ((r3 - 128) << 12), actor->y + ((r2 - 128) << 12),
...
void A_LightningClip(mobj_t *actor) {
  ...
  if (actor->type ==
      MT_LIGHTNING_FLOOR) { // floor lightning zig-zags, and forces the ceiling
                            // lightning to mimic
...
void A_MStaffPalette(player_t *player, pspdef_t *psp) {
  ...
  if (player == &players[consoleplayer]) {
    ...
    if (pal == STARTSCOURGEPAL + 3) { // reset back to original playpal
      pal = 0;
    }
    I_SetPalette((byte *)W_CacheLumpNum(W_GetNumForName("playpal"), PU_CACHE) +
                 pal * 768);
...
void A_CFlameMissile(mobj_t *actor) {
  ...
  S_StartSound(actor, SFX_CLERIC_FLAME_EXPLODE);
  if (BlockingMobj &&
      BlockingMobj->flags & MF_SHOOTABLE) { // Hit something, so spawn the flame
                                            // circle around the thing
...
void A_CFlameAttack(player_t *player, pspdef_t *psp)
{
        ...
        { // Hit something, so spawn the flame circle around the thing
                ...
                for(i = 0; i < 4; i++)
                {
                        ...
                        mo = P_SpawnMobj(linetarget->x+FixedMul(dist,
finecosine[an]), linetarget->y+FixedMul(dist, finesine[an]),
...
finecosine[an]); mo->momy = mo->special2.i = FixedMul(FLAMESPEED, finesine[an]);
                                mo->tics -= P_Random()&3;
                        }
                        mo = P_SpawnMobj(linetarget->x-FixedMul(dist,
finecosine[an]), linetarget->y-FixedMul(dist, finesine[an]),
...
FixedMul(-FLAMESPEED, finecosine[an]); mo->momy = mo->special2.i =
FixedMul(-FLAMESPEED, finesine[an]); mo->tics -= P_Random()&3;
                        }
                }
        }
// Create a line of flames from the player to the flame puff
        ...
        player->mana[MANA_2] -=
WeaponManaUse[player->class][player->readyweapon]; S_StartSound(player->mo,
...
void A_CHolyAttack2(mobj_t *actor) {
  ...
  for (j = 0; j < 4; j++) {
    ...
    case 3:
      r = P_Random();
      mo->special2.i = ((32 + (r & 7)) << 16) + 32 + (P_Random() & 7);
      break;
    }
    mo->z = actor->z;
    ...
    if (deathmatch) { // Ghosts last slightly less longer in DeathMatch
      mo->health = 85;
    }
    if (linetarget) {
...
void A_CHolyPalette(player_t *player, pspdef_t *psp) {
  ...
  if (player == &players[consoleplayer]) {
    ...
    if (pal == STARTHOLYPAL + 3) { // reset back to original playpal
      pal = 0;
    }
    I_SetPalette((byte *)W_CacheLumpNum(W_GetNumForName("playpal"), PU_CACHE) +
                 pal * 768);
...
void A_CHolySeek(mobj_t *actor) {
  ...
  if (actor->special1.m) {
    CHolySeekerMissile(actor, actor->args[0] * ANG1, actor->args[0] * ANG1 * 2);
    if (!((leveltime + 7) & 15)) {
...
static void CHolyTailFollow(mobj_t *actor, fixed_t dist) {
  ...
  if (child) {
    ...
    if (P_TryMove(child, actor->x + FixedMul(dist, finecosine[an]),
      ...
      newDistance =
          P_AproxDistance(child->x - actor->x, child->y - actor->y) - FRACUNIT;
...
void A_CHolyTail(mobj_t *actor) {
  ...
  if (parent) {
    if (parent->state >=
        &states[parent->info->deathstate]) { // Ghost removed, so remove all
                                             // tail parts
...
void A_FireConePL1(player_t *player, pspdef_t *psp) {
  ...
  if (!conedone) {
    ...
    if (mo) {
      mo->special1.i =
          SHARDSPAWN_LEFT | SHARDSPAWN_DOWN | SHARDSPAWN_UP | SHARDSPAWN_RIGHT;
...
void A_ShedShard(mobj_t *actor) {
  ...
  if (spawndir & SHARDSPAWN_LEFT) {
    mo =
        P_SpawnMissileAngleSpeed(actor, MT_SHARDFX1, actor->angle + (ANG45 / 9),
