...
EM_BOOL WebSocketOpen(int eventType, const EmscriptenWebSocketOpenEvent *e,
                      void *userData) {
  return 0;
}

EM_BOOL WebSocketClose(int eventType, const EmscriptenWebSocketCloseEvent *e,
                       void *userData) {
  printf("doom: 5, ws close(eventType=%d, wasClean=%d, code=%d, reason=%s, "
         "userData=%d)\n",
...
EM_BOOL WebSocketError(int eventType, const EmscriptenWebSocketErrorEvent *e,
                       void *userData) {
...
EM_BOOL WebSocketMessage(int eventType,
                         const EmscriptenWebSocketMessageEvent *e,
                         void *userData) {
  // printf("message(eventType=%d, userData=%d, data=%p, numBytes=%d,
...
static boolean InitWebSockets(void) {
  ...
  if (wss) {
    ...
    emscripten_websocket_set_onopen_callback(websocket, (void *)42,
                                             WebSocketOpen);
    emscripten_websocket_set_onclose_callback(websocket, (void *)43,
                                              WebSocketClose);
    emscripten_websocket_set_onerror_callback(websocket, (void *)44,
                                              WebSocketError);
    emscripten_websocket_set_onmessage_callback(websocket, (void *)45,
                                                WebSocketMessage);
...
static net_addr_t *FindAddressByIp(uint32_t ip) {
  ...
  // i'm assuming there won't be more than MAX_QUEUE_SIZE address during one
  // gaming session
...
net_module_t net_websockets_module = {
    NET_Websockets_InitClient,     NET_Websockets_InitServer,
    NET_Websockets_SendPacket,     NET_Websockets_RecvPacket,
    NET_Websockets_AddrToString,   NET_Websockets_FreeAddress,
