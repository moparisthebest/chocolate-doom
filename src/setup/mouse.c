...
static int *all_mouse_buttons[] = {
    &mousebfire,        &mousebstrafe,     &mousebforward, &mousebstrafeleft,
    &mousebstraferight, &mousebbackward,   &mousebuse,     &mousebjump,
    &mousebprevweapon,  &mousebnextweapon, &mousebinvleft, &mousebinvright,
    &mousebturnleft,    &mousebturnright,
};
...
static void ConfigExtraButtons(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused)) {
  ...
  TXT_AddWidgets(window, buttons_table = TXT_NewTable(4), NULL);
...
void ConfigMouse(TXT_UNCAST_ARG(widget), void *user_data) {
  ...
  TXT_AddWidgets(
      window, TXT_NewCheckBox("Enable mouse", &usemouse),
      ...
      TXT_NewSeparator("Mouse motion"), TXT_NewLabel("Speed"),
