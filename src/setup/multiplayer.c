...
static const char *chex_skills[] = {"Easy does it", "Not so sticky",
                                    "Gobs of goo", "Extreme ooze",
                                    "SUPER SLIMEY!"};
...
static const char *hexen_fighter_skills[] = {"Squire", "Knight", "Warrior",
                                             "Berserker", "Titan"};

static const char *hexen_cleric_skills[] = {"Altar boy", "Acolyte", "Priest",
                                            "Cardinal", "Pope"};

static const char *hexen_mage_skills[] = {"Apprentice", "Enchanter", "Sorceror",
                                          "Warlock", "Archimage"};

static const char *strife_skills[] = {"Training", "Rookie", "Veteran", "Elite",
                                      "Bloodbath"};
...
static void StartSinglePlayerGame(TXT_UNCAST_ARG(widget),
...
static void CloseLevelSelectDialog(TXT_UNCAST_ARG(button),
...
static void OpenWadsWindow(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(user_data)) {
  ...
  for (i = 0; i < NUM_WADS; ++i) {
    TXT_AddWidget(window, TXT_NewFileSelector(&wads[i], 60, "Select a WAD file",
                                              wad_extensions));
  }
}
...
static void StartGameMenu(const char *window_title, int multiplayer) {
  ...
  TXT_AddWidgets(window, TXT_NewLabel("Game"), iwad_selector = IWADSelector(),
                 NULL);

  if (gamemission == hexen) {
    txt_dropdown_list_t *cc_dropdown;
    TXT_AddWidgets(window, TXT_NewLabel("Character class "),
  ...
  TXT_AddWidgets(window, TXT_NewLabel("Skill"),
  ...
  if (multiplayer) {
    TXT_AddWidgets(window, TXT_NewLabel("Game type"), GameTypeDropdown(),
  ...
  TXT_AddWidgets(window, TXT_NewSeparator("Monster options"),
  ...
  if (multiplayer) {
    TXT_AddWidgets(
        window, TXT_NewSeparator("Advanced"), TXT_NewLabel("UDP port"),
...
static void QueryResponseCallback(net_addr_t *addr, net_querydata_t *querydata,
  ...
  TXT_AddWidgets(
      results_table, TXT_NewLabel(ping_time_str),
      TXT_NewButton2(NET_AddrToString(addr), SelectQueryAddress, querydata),
      TXT_NewLabel(description), NULL);
...
static void QueryPeriodicCallback(TXT_UNCAST_ARG(results_table)) {
  ...
  if (!NET_Query_Poll(QueryResponseCallback, results_table)) {
    ...
    if (query_servers_found == 0) {
      TXT_AddWidgets(results_table, TXT_TABLE_EMPTY,
                     TXT_NewLabel("No compatible servers found."), NULL);
    }
  }
}
...
static void FindLANServer(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(user_data)) {
  NET_StartLANQuery();
...
void JoinMultiGame(TXT_UNCAST_ARG(widget), void *user_data) {
  ...
  TXT_AddWidgets(window, TXT_NewLabel("Game"), IWADSelector(), NULL);

  if (gamemission == hexen) {
    TXT_AddWidgets(window, TXT_NewLabel("Character class "),
                   TXT_NewDropdownList(&character_class, character_classes, 3),
                   NULL);
  }

  TXT_AddWidgets(
      window, TXT_NewSeparator("Server"), TXT_NewLabel("Connect to address: "),
...
void MultiplayerConfig(TXT_UNCAST_ARG(widget), void *user_data) {
  ...
  TXT_AddWidgets(window, TXT_NewStrut(0, 1),
                 TXT_NewHorizBox(TXT_NewLabel("Player name:  "),
                                 TXT_NewInputBox(&net_player_name, 25), NULL),
                 TXT_NewStrut(0, 1), TXT_NewSeparator("Chat macros"), NULL);
