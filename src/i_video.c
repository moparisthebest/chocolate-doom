...
void I_DisplayFPSDots(boolean dots_on) { display_fps_dots = dots_on; }
...
void I_SetWindowTitle(const char *title) { window_title = title; }
...
static void SetVideoMode(void) {
  ...
  if (argbbuffer == NULL) {
    SDL_PixelFormatEnumToMasks(pixel_format, &bpp, &rmask, &gmask, &bmask,
                               &amask);
    argbbuffer = SDL_CreateRGBSurface(0, SCREENWIDTH, SCREENHEIGHT, bpp, rmask,
                                      gmask, bmask, amask);
