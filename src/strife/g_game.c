...
static const struct {
  ...
  weapontype_t weapon_num;
    ...
    {wp_fist, wp_fist},           {wp_poisonbow, wp_elecbow},
    {wp_elecbow, wp_elecbow},     {wp_rifle, wp_rifle},
    {wp_missile, wp_missile},     {wp_wpgrenade, wp_hegrenade},
    {wp_hegrenade, wp_hegrenade}, {wp_flame, wp_flame},
    {wp_torpedo, wp_mauler},      {wp_mauler, wp_mauler},
    {wp_sigil, wp_sigil},
};
...
void G_DoLoadLevel(void) {
  ...
  for (i = 0; i < MAXPLAYERS; i++) {
    ...
    if (playeringame[i] &&
        (players[i].playerstate == PST_DEAD || players[i].health <= 0))
...
boolean G_Responder(event_t *ev) {
  ...
  if (gamestate == GS_LEVEL && ev->type == ev_keydown && ev->data1 == key_spy &&
      (singledemo || !gameskill)) // [STRIFE]: o_O
  {
    // spy mode
    do {
      displayplayer++;
  ...
  while (gameaction != ga_nothing) {
    ...
    case ga_screenshot:
      ...
      players[consoleplayer].message =
          DEH_String("STRIFE  by Rogue entertainment");
      gameaction = ga_nothing;
      break;
    case ga_nothing:
      break;
    ...
    if (playeringame[i]) {
      if (players[i].cmd.buttons & BT_SPECIAL) {
        ...
        case BTS_SAVEGAME:
          ...
          {
            M_StringCopy(character_name, "NET GAME", sizeof(character_name));
          }
          savegameslot =
...
void G_PlayerReborn(int player) {
  ...
  M_StringCopy(mission_objective, DEH_String("Find help"), OBJECTIVE_LEN);
}
...
// W1 Start Finale.
//
void G_StartFinale(void) { gameaction = ga_victory; }
...
void G_RiftPlayer(void) {
  if (riftdest) {
    P_TeleportMove(players[0].mo, riftSpots[riftdest - 1].x << FRACBITS,
...
boolean G_RiftCheat(int riftSpotNum) {
  return P_TeleportMove(players[0].mo, riftSpots[riftSpotNum - 1].x << FRACBITS,
...
void G_ReadCurrent(const char *path) {
  ...
  if (M_ReadFile(temppath, &buffer) <= 0)
    ...
    gamemap = (((int)buffer[0]) | ((int)buffer[1] << 8) |
