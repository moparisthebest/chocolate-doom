...
menu_t MainDef = {main_end, NULL, MainMenu, M_DrawMainMenu,
...
enum { rdthsempty3, read3_end } read_e3;

menuitem_t ReadMenu3[] = {{1, "", M_ClearMenus, 0}};

menu_t ReadDef3 = {read3_end, &ReadDef2, ReadMenu3, M_DrawReadThis3,
                   250,       185,       0};
...
menuitem_t SoundMenu[] = {
    {2, "M_SFXVOL", M_SfxVol, 's'},           {-1, "", 0, '\0'},
    {2, "M_MUSVOL", M_MusicVol, 'm'},         {-1, "", 0, '\0'},
    {2, "M_VOIVOL", M_VoiceVol, 'v'},         {-1, "", 0, '\0'},
    {2, "M_MSENS", M_ChangeSensitivity, 'm'}, {-1, "", 0, '\0'}};

menu_t SoundDef = {sound_end, &OptionsDef, SoundMenu, M_DrawSound,
...
//
menu_t NameCharDef = {load_end, &NewDef, SaveMenu, M_DrawNameChar, 80, 54, 0};
...
void M_DrawSound(void) {
  ...
  M_DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT * (voice_vol + 1), 16,
               voiceVolume);

  M_DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT * (sfx_mouse + 1), 16,
               mouseSensitivity);
}
...
//
void M_DrawMainMenu(void) {
  V_DrawPatchDirect(84, 2, W_CacheLumpName(DEH_String("M_STRIFE"), PU_CACHE));
}
...
void M_DrawEpisode(void)
{
    V_DrawPatchDirect(54, 38, W_CacheLumpName(DEH_String("M_EPISOD"),
PU_CACHE));
}
...
void M_DrawOptions(void) {
  ...
  V_DrawPatchDirect(108, 15, W_CacheLumpName(DEH_String("M_OPTION"), PU_CACHE));
...
// decompiler could not touch it. Redimensions a string to fit on screen,
// leaving at least a 20 pixel margin on the right side. The string passed in
// must be writable.
//
...
void M_Drawer(void) {
  ...
  // DRAW SIGIL
  V_DrawPatchDirect(
      x + CURSORXOFF, currentMenu->y - 5 + itemOn * LINEHEIGHT,
      W_CacheLumpName(DEH_String(cursorName[whichCursor]), PU_CACHE));
}
