...
boolean PIT_CheckThing(mobj_t *thing) {
  ...
  if (tmthing->flags & MF_MISSILE) {
    // villsa [STRIFE] code to check over/under clipping moved to the beginning
    // of the function, so that it applies to everything.
...
boolean PTR_ShootTraverse(intercept_t *in) {
  ...
  if (in->d.thing->flags & MF_NOBLOOD)
    P_SpawnSparkPuff(x, y,
                     z); // villsa [STRIFE] call spark puff function instead
  else
