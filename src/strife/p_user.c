  ...
  */
  ...
  if (cmd->buttons & BT_CHANGE) {
    ...
    if (newweapon == wp_elecbow) {
      if (player->weaponowned[wp_poisonbow] &&
          player->readyweapon == wp_elecbow) {
    ...
    if (newweapon == wp_hegrenade) {
      if (player->weaponowned[wp_wpgrenade] &&
          player->readyweapon == wp_hegrenade) {
    ...
    if (player->weaponowned[newweapon] && newweapon != player->readyweapon) {
      ...
      if (weaponinfo[newweapon].availabledemo || !isdemoversion) {
        if (player->ammo[weaponinfo[newweapon].ammo])
          ...
          if (newweapon == wp_elecbow && player->ammo[am_poisonbolts] &&
          ...
          else if (newweapon == wp_hegrenade && player->ammo[am_wpgrenades] &&
  ...
  if (player->powers[pw_targeter]) {
    ...
    if (player->powers[pw_targeter] == 1) {
      ...
      } else if (player->powers[pw_targeter] &
                 16) // haleyjd 20110205: missing else
      {
...
void P_DropInventoryItem(player_t *player, int sprite) {
  ...
  while (invslot < player->numinventory &&
         sprite != player->inventory[invslot].sprite)
    invslot++;
...
boolean P_UseInventoryItem(player_t *player, int item) {
  ...
  for (i = 0; i < player->numinventory; i++) {
    ...
    M_snprintf(useinventorymsg, sizeof(useinventorymsg), "You used the %s.",
               name);
