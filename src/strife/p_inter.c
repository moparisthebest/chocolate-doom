...
void P_TouchSpecialThing(mobj_t *special, mobj_t *toucher) {
  ...
  // grenade launcher
  case SPR_GRND:
    if (!P_GiveWeapon(player, wp_hegrenade, (special->flags & MF_DROPPED) != 0))
      return;
    sound = sfx_wpnup; // haleyjd: SHK-CHK!
    break;
  ...
  case SPR_CBOW:
    if (!P_GiveWeapon(player, wp_elecbow, (special->flags & MF_DROPPED) != 0))
      return;
    sound = sfx_wpnup; // haleyjd: SHK-CHK!
    break;
  ...
  if (!pickupmsg[0]) {
    if (special->info->name) {
      DEH_snprintf(pickupmsg, sizeof(pickupmsg), "You picked up the %s.",
...
void P_KillMobj(mobj_t *source, mobj_t *target) {
  ...
  // villsa [STRIFE] corpse and dropoff are removed, but why when these two
  // flags are set a few lines later? watcom nonsense perhaps?
  target->flags &=
      ~(MF_SHOOTABLE | MF_FLOAT | MF_BOUNCE | MF_CORPSE | MF_DROPOFF);
  ...
  */
  ...
  if (source && source->player) {
    ...
    if (target->player) {
      ...
      DEH_snprintf(plrkilledmsg, sizeof(plrkilledmsg), "%s killed %s",
  ...
  if (target->player) {
    ...
    if (netgame) {
      ...
      while (1) {
        ...
        if (item == MT_MONY_1) {
          loot = P_SpawnMobj(target->x, target->y, target->z + (24 * FRACUNIT),
                             MT_MONY_25);
  ...
  case MT_TOKEN_SHOPCLOSE:
    ...
    M_snprintf(plrkilledmsg, sizeof(plrkilledmsg), "%s",
               DEH_String("You're dead!  You set off the alarm."));
    if (!deathmatch)
      players[consoleplayer].message = plrkilledmsg;
  ...
  case MT_TOKEN_ALARM:
    ...
    M_snprintf(plrkilledmsg, sizeof(plrkilledmsg), "%s",
               DEH_String("You Fool!  You've set off the alarm"));
    if (!deathmatch)
      players[consoleplayer].message = plrkilledmsg;
    return;
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source,
  ...
  if (inflictor) {
    ...
    if (inflictor->type == MT_SFIREBALL || inflictor->type == MT_C_FLAME ||
      ...
      case MT_HOOKSHOT:
        ...
        target->momx +=
            FixedMul(finecosine[ang], (12750 * FRACUNIT) / target->info->mass);
        target->momy +=
            FixedMul(finesine[ang], (12750 * FRACUNIT) / target->info->mass);
      ...
      default:
        ...
        if (target->flags & MF_SPECTRAL && !(inflictor->flags & MF_SPECTRAL)) {
  ...
  if (inflictor && !(target->flags & MF_NOCLIP) &&
      (!source || !source->player || source->player->readyweapon != wp_flame)) {
  ...
  if (target->health <= 0) {
    ...
    if (inflictor && inflictor->type == MT_HEGRENADE)
      ...
      if (inflictor &&
          (inflictor->type == MT_STRIFEPUFF3 || inflictor->type == MT_L_LASER ||
    ...
    if (!(target->flags & MF_NOBLOOD) && inflictor &&
        (inflictor->type == MT_SFIREBALL || inflictor->type == MT_C_FLAME ||
         inflictor->type == MT_PFLAME)) {
  ...
  if (target->type != MT_PROGRAMMER &&
      (!target->threshold || target->type == MT_ENTITY) && source &&
      source != target && source->type != MT_ENTITY &&
