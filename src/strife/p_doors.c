...
void T_VerticalDoor(vldoor_t *door) {
  ...
  case 1:
    // UP
    res1 = T_MovePlane(door->sector, door->speed, door->topheight, false, 1,
                       door->direction);
...
void EV_VerticalDoor(line_t *line, mobj_t *thing) {
  ...
  case 34: // D1 Pass Card door
    if (!player->cards[key_Passcard]) {
      player->message =
          DEH_String("You need a pass card key to open this door");
      S_StartSound(NULL, sfx_oof);
      return;
    }
    break;
...
void EV_SlidingDoor(line_t *line, mobj_t *thing) {
  ...
  if (!door) {
    ...
    if (door->whichDoorIndex < 0)
      I_Error(DEH_String(
          "EV_SlidingDoor: Textures are not defined for sliding door!"));
    ...
    T_MovePlane(door->frontsector, (128 * FRACUNIT),
                P_FindLowestCeilingSurrounding(door->frontsector), 0, 1, 1);

    // villsa [STRIFE] play open sound
    S_StartSound(&door->frontsector->soundorg,
                 slideOpenSounds[door->whichDoorIndex]);
  }
}
