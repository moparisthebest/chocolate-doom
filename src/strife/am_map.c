...
static int mapmarknum = 0;   // villsa [STRIFE] unused but this was meant to be
                             // used for objective based markers
...
void AM_addMark(void) {
  markpoints[markpointnum].x =
      plr->mo->x; // 20160306 [STRIFE]: use player position
  ...
  if (!automapactive) {
    ...
    */
      ...
      if (markpointnum == AM_NUMMARKPOINTS)
        --markpointnum;
      M_snprintf(buffer, sizeof(buffer), "%s %d", DEH_String(AMSTR_MARKEDSPOT),
                 markpointnum + 1); // [STRIFE]
      plr->message = buffer;
      AM_addMark();
  ...
  for (i = 0; i < numlines; i++) {
    ...
    if (cheating || (line->flags & ML_MAPPED)) {
      ...
      // villsa [STRIFE] lightlev is unused here
        ...
        if (line->special == 39) { // teleporters
...
void AM_drawPlayers(void) {
  ...
  static int their_colors[] = {0x80, 0x40, 0xB0, 0x10, 0x30,
                               0x50, 0xA0, 0x60, 0x90};
  int their_color = -1;
  int color;
...
void AM_drawThings(void) {
  ...
  for (i = 0; i < numsectors; i++) {
    ...
    while (t) {
      ...
      AM_drawLineCharacter(thintriangle_guy, arrlen(thintriangle_guy), radius,
                           t->angle, colors, t->x, t->y);
