...
const char *mapnames[] = {
    ...
    HUSTR_10, HUSTR_11, HUSTR_12, HUSTR_13, HUSTR_14, HUSTR_15, HUSTR_16,
    HUSTR_17, HUSTR_18, HUSTR_19,

    // Third "episode" - Shut down Factory, kill Loremaster and Entity
    HUSTR_20, HUSTR_21, HUSTR_22, HUSTR_23, HUSTR_24, HUSTR_25, HUSTR_26,
    HUSTR_27, HUSTR_28, HUSTR_29,

    // "Secret" levels - Abandoned Base and Training Facility
    HUSTR_30, HUSTR_31,

    // Demo version maps
    HUSTR_32, HUSTR_33, HUSTR_34};
        ...
        if (c <= HU_CHANGENAME) // [STRIFE]: allow HU_CHANGENAME here
          ...
          if (rc && c == KEY_ENTER) {
            if (w_inputbuffer[i].l.len && (chat_dest[i] == consoleplayer + 1 ||
              ...
              HU_addMessage(player_names[i], w_inputbuffer[i].l.l);
    ...
    if (altdown) {
      ...
      if (w_chat.l.len) // [STRIFE]: past first char of chat?
      {
        ...
        if (i < MAXPLAYERS) {
          ...
          if (i == consoleplayer) {
            ...
            DEH_snprintf(lastmessage, sizeof(lastmessage), "Talking to: %c",
                         '1' + i);
            plr->message = lastmessage;
          }
      ...
      if (c == KEY_ENTER) {
        ...
        if (w_chat.l.len) {
          // [STRIFE]: name setting
          if (hu_setting_name) {
            DEH_snprintf(lastmessage, sizeof(lastmessage), "%s now %.13s",
                         player_names[consoleplayer], w_chat.l.l);
