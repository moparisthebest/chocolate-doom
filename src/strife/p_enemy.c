...
void P_DoPunchAlert(mobj_t *puncher, mobj_t *punchee) {
  ...
  for (rover = punchee->snext; rover; rover = rover->snext) {
    ...
    if (rover->health > 0 && rover->type >= MT_GUARD1 &&
        rover->type <= MT_PGUARD &&
...
void A_SentinelAttack(mobj_t *actor) {
  ...
  if (mo->momy | mo->momx) // villsa - fixed typo (yes, they actually used '|'
                           // instead of'||')
  {
    for (i = 8; i > 1; i--) {
...
//
void A_StalkerDrop(mobj_t *actor) { actor->flags &= ~MF_NOGRAVITY; }
...
void A_InqTakeOff(mobj_t *actor) {
  ...
  dist =
      P_AproxDistance(actor->target->x - actor->x, actor->target->y - actor->y);
...
void A_SpectreCAttack(mobj_t *actor) {
  ...
  mo = P_SpawnMobj(actor->x, actor->y, actor->z + (32 * FRACUNIT),
...
//
void A_ShadowOff(mobj_t *actor) { actor->flags &= ~(MF_SHADOW | MF_MVIS); }
...
//
void A_ShadowOn(mobj_t *actor) { actor->flags |= (MF_SHADOW | MF_MVIS); }
...
void A_FireChainShot(mobj_t *actor) {
  ...
  P_SpawnMobj(actor->x, actor->y, actor->z,
              MT_CHAINSHOT); // haleyjd: fixed type

  P_SpawnMobj(actor->x - (actor->momx >> 1), actor->y - (actor->momy >> 1),
              actor->z, MT_CHAINSHOT);

  P_SpawnMobj(actor->x - actor->momx, actor->y - actor->momy, actor->z,
              MT_CHAINSHOT);
}
...
int TRACEANGLE =
    0xE000000; // villsa [STRIFE] changed from 0xC000000 to 0xE000000
...
void A_ProgrammerDie(mobj_t *actor) {
  ...
  mo = P_SpawnMobj(actor->x, actor->y, actor->z + 24 * FRACUNIT,
                   MT_PROGRAMMERBASE);
...
void P_DestroyConverter(void) {
  ...
  DEH_snprintf(pmsgbuffer, sizeof(pmsgbuffer),
               "You've destroyed the Converter!");
...
void A_HeadChunk(mobj_t *actor) {
  ...
  mo =
      P_SpawnMobj(actor->x, actor->y, actor->z + 10 * FRACUNIT, MT_SPECTREHEAD);
  r = P_Random();
...
void A_TeleportBeacon(mobj_t *actor) {
  ...
  fog_x = mobj->x +
          FixedMul(20 * FRACUNIT, finecosine[actor->angle >> ANGLETOFINESHIFT]);
  fog_y = mobj->y +
          FixedMul(20 * FRACUNIT, finesine[actor->angle >> ANGLETOFINESHIFT]);
