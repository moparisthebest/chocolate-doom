...
#define DIALOG_INT(field, ptr)                                                 \
  field = ((int)ptr[0] | ((int)ptr[1] << 8) | ((int)ptr[2] << 16) |            \
           ((int)ptr[3] << 24));                                               \
  ptr += 4;
...
static menuitem_t dialogmenuitems[] = {
    ...
    {1, "", P_DialogDoChoice, '2'}, {1, "", P_DialogDoChoice, '3'},
    {1, "", P_DialogDoChoice, '4'}, {1, "", P_DialogDoChoice, '5'},
    {1, "", P_DialogDoChoice, '6'} // Item 6 is always the dismissal item
};

static menu_t dialogmenu = {
    NUMDIALOGMENUITEMS, NULL, dialogmenuitems, P_DialogDrawer, 42, 75, 0};
...
//
static void P_ParseDialogLump(byte *lump, mapdialog_t **dialogs, int numdialogs,
                              int tag) {
  int i;
  byte *rover = lump;
...
boolean P_GiveItemToPlayer(player_t *player, int sprnum, mobjtype_t type) {
  ...
  case SPR_CHST: // 50 gold
    for (i = 0; i < 50; i++)
      P_GiveInventoryItem(player, SPR_COIN, MT_MONY_1);
    break; // haleyjd 20141215: missing break, caused Rowan to not take ring
           // from you.
...
static void P_DialogDrawer(void) {
  ...
  angle = R_PointToAngle2(dialogplayer->mo->x, dialogplayer->mo->y,
                          dialogtalker->x, dialogtalker->y);
  ...
  if (dialogbgpiclumpnum == -1 || menupausetime <= gametic) {
    ...
    for (i = 0; i < dialogmenu.numitems - 1; i++) {
      DEH_snprintf(choicetext, sizeof(choicetext), "%d) %s", i + 1,
                   currentdialog->choices[i].text);
      ...
      if (currentdialog->choices[i].needamounts[0] > 0) {
        ...
        DEH_snprintf(choicetext, sizeof(choicetext), "%s for %d", choicetext2,
...
void P_DialogDoChoice(int choice) {
  ...
  I_StartVoice(
      NULL); // STRIFE-TODO: verify (should stop previous voice I believe)
  ...
  if (choice != dialogmenu.numitems - 1 && candochoice) {
    ...
    if (item < 0 ||
        P_GiveItemToPlayer(dialogplayer,
                           states[mobjinfo[item].spawnstate].sprite, item)) {
      // if successful, take needed items
      int count = 0;
      // villsa 09/08/10: converted into for loop
      for (count = 0; count < MDLG_MAXITEMS; count++) {
        P_TakeDialogItem(dialogplayer, currentchoice->needitems[count],
...
//
void P_DialogStartP1(void) { P_DialogStart(&players[0]); }
...
void P_DialogStart(player_t *player) {
  ...
  if (!linetarget) {
    P_AimLineAttack(player->mo, player->mo->angle + (ANG90 / 16),
                    (128 * FRACUNIT));
    if (!linetarget)
      P_AimLineAttack(player->mo, player->mo->angle - (ANG90 / 16),
                      (128 * FRACUNIT));
  ...
  DEH_snprintf(dialoglastmsgbuffer, sizeof(dialoglastmsgbuffer), "%d) %s",
               i + 1, byetext);
}
