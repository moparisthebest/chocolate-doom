...
void P_ZMovement(mobj_t *mo) {
  ...
  if (mo->z <= mo->floorz) {
    ...
    */
    ...
    if ((mo->flags & MF_MISSILE) && !(mo->flags & (MF_NOCLIP | MF_BOUNCE))) {
      P_ExplodeMissile(mo);
    }
...
void P_MobjThinker(mobj_t *mobj) {
  ...
  if ((mobj->z != mobj->floorz &&
       !(mobj->flags & MF_NOGRAVITY)) // villsa [STRIFE]
      || mobj->momz) {
    P_ZMovement(mobj);
...
mobj_t *P_SpawnMissile(mobj_t *source, mobj_t *dest, mobjtype_t type) {
  ...
  if (dest->flags & MF_SHADOW) {
    int t =
        P_Random(); // haleyjd 20110223: remove order-of-evaluation dependencies
    an += (t - P_Random()) << 21;
  }
