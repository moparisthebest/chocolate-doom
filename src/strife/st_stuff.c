...
static const char *invammonames[NUMAMMO] = {
    "I_BLIT", "I_XQRL", "I_PQRL", "I_BRY1", "I_ROKT", "I_GRN1", "I_GRN2"};
...
cheatseq_t cheat_keys =
    CHEAT("jimmy", 0); // [STRIFE]: new cheat jimmy (all keys)
...
cheatseq_t cheat_powerup[NUM_ST_PUMPUP] = // [STRIFE]
    {
        CHEAT("pumpupb", 0), CHEAT("pumpupi", 0), CHEAT("pumpupm", 0),
        CHEAT("pumpuph", 0), CHEAT("pumpupp", 0), CHEAT("pumpups", 0),
        CHEAT("pumpupt", 0), CHEAT("pumpup", 0),
};
...
boolean ST_Responder(event_t *ev) {
  ...
  if (ev->type == ev_keyup) {
    ...
    // villsa [STRIFE]
    if (ev->data1 != key_invpop && ev->data1 != key_mission &&
        ev->data1 != key_invkey)
      return false;
  ...
  if (ev->data1 == key_invquery) // inventory query
  {
    ...
    if (inv->amount) {
      DEH_snprintf(st_msgbuf, sizeof(st_msgbuf), "%d %s", inv->amount,
  ...
  // villsa [STRIFE]
  if (ev->data1 == key_invpop || ev->data1 == key_invkey ||
...
void ST_doPaletteStuff(void) {
  ...
  // instead).
  else if (plyr->nukagecount > 16 * TICRATE || (plyr->nukagecount & 8))
...
void ST_doRefresh(void) {
  ...
  if (st_statusbaron) {
    ...
    // Draw cursor.
    if (plyr->numinventory) {
      V_DrawPatch(35 * (plyr->inventorycursor - firstinventory) + 42, 180,
                  invcursor);
    }

    // Draw inventory bar
    for (num_x = 68, icon_x = 48, i = firstinventory, numdrawn = 0; num_x < 278;
  ...
  if (deathmatch) {
    ...
    if (st_keypage > 0) {
      ...
      for (key = ST_KEYSPERPAGE * st_keypage, keycount = 0;
           keycount < ST_KEYSPERPAGE && key < NUMCARDS; ++key, ++keycount) {
        if (plyr->cards[key])
          haskeyinrange = true;
      }
    ...
    for (; keycount < ST_KEYSPERPAGE && key < NUMCARDS;
         ++key, ++keycount, ++info) {
      char sprname[8];
      patch_t *patch;
...
void ST_loadGraphics(void) { ST_loadUnloadGraphics(ST_loadCallback); }
...
void ST_unloadGraphics(void) { ST_loadUnloadGraphics(ST_unloadCallback); }
...
void ST_createWidgets(void) {
  ...
  // ready weapon ammo
  STlib_initNum(&w_ready, ST_AMMOX, ST_AMMOY, invfontg,
                &plyr->ammo[weaponinfo[plyr->readyweapon].ammo], ST_AMMOWIDTH);

  // the last weapon type
  w_ready.data = plyr->readyweapon;

  // health percentage
  STlib_initNum(&w_health, ST_HEALTHX, ST_HEALTHY, invfontg, &plyr->health,
                ST_HEALTHWIDTH);
  ...
  for (i = 0; i < NUMAMMO; i++) {
    STlib_initNum(&w_ammo[i], ST_POPUPAMMOX, st_yforammo[i], invfonty,
                  &plyr->ammo[i], st_wforammo[i]);

    STlib_initNum(&w_maxammo[i], ST_POPUPMAXAMMOX, st_yforammo[i], invfonty,
                  &plyr->maxammo[i], st_wforammo[i]);
  }
}
