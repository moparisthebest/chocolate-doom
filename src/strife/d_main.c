...
void D_Display(void) {
  ...
  do {
    ...
    wipestart = nowtime;
        ...
        wipe_ScreenWipe(wipe_ColorXForm, 0, 0, SCREENWIDTH, SCREENHEIGHT, tics);
    I_UpdateNoBlit();
...
static boolean D_StartupGrabCallback(void) { return false; }
...
static char *GetGameName(char *gamename) {
  ...
  for (i = 0; i < arrlen(banners); ++i) {
    ...
    if (deh_sub != banners[i]) {
      ...
      M_snprintf(gamename, gamename_size, deh_sub, STRIFE_VERSION / 100,
                 STRIFE_VERSION % 100);
...
} gameversions[] = {{"Strife 1.2", "1.2", exe_strife_1_2},
...
boolean D_PatchClipCallback(patch_t *patch, int x, int y) {
  ...
  return (x >= 0 && y >= 0 && x + SHORT(patch->width) <= SCREENWIDTH &&
...
void D_DoomMain(void) {
  ...
  if (modifiedgame) {
    ...
    char name[3][8] = {"map23", "map30", "ROB3E1"};
    int i;
