...
void A_WeaponReady(player_t *player, pspdef_t *psp) {
  ...
  if (player->readyweapon == wp_flame && psp->state == &states[S_FLMT_00]) // 62
  {
  ...
  if (player->cmd.buttons & BT_ATTACK) {
    if (!player->attackdown ||
        (player->readyweapon != wp_missile &&
         player->readyweapon !=
             wp_torpedo)) // villsa [STRIFE] replace bfg with torpedo
    {
...
void A_FireGrenade(player_t *player, pspdef_t *pspr) {
  ...
  mo->momz = FixedMul((player->pitch << FRACBITS) / 160, mo->info->speed) +
             (8 * FRACUNIT);
...
// villsa [STRIFE] - new codepointer
//
void A_SigilShock(player_t *player, pspdef_t *psp) { player->extralight = -3; }
