...
typedef enum { SINGLE, COOPERATIVE, DEATHMATCH } gametype_t;
...
void IN_InitStats(void) {
  ...
  if (!netgame) {
    ...
    if (playercount ==
        slaughtercount) { // don't do the slaughter stuff if everyone is equal
      slaughterboy = 0;
    }
  }
}
...
static void UnloadLumpCallback(const char *lumpname, int lumpnum,
                               patch_t **ptr) {
  if (lumpname != NULL) {
...
void IN_UnloadPics(void) { IN_LoadUnloadPics(UnloadLumpCallback); }
...
void IN_Ticker(void) {
  ...
  if (oldintertime < intertime) {
    ...
    if (gameepisode > 3 &&
        interstate >=
            1) { // Extended Wad levels:  skip directly to the next level
      interstate = 3;
    }
    switch (interstate) {
    case 0:
  ...
  if (prevmap == 9) {
    ...
    if (!(intertime & 16)) {
      V_DrawPatch(YAHspot[gameepisode - 1][prevmap - 1].x,
                  YAHspot[gameepisode - 1][prevmap - 1].y, patchBEENTHERE);
    }
  }
}
...
void IN_DrawYAH(void) {
  ...
  if (!(intertime & 16) || interstate == 3) { // draw the destination 'X'
...
void IN_DrawDMStats(void) {
  ...
  if (intertime < 20) {
    for (i = 0; i < MAXPLAYERS; i++) {
      if (playeringame[i]) {
        V_DrawShadowedPatch(
            40, ((ypos << FRACBITS) + dSlideY[i] * intertime) >> FRACBITS,
            W_CacheLumpNum(patchFaceOkayBase + i, PU_CACHE));
        V_DrawShadowedPatch(
            ((xpos << FRACBITS) + dSlideX[i] * intertime) >> FRACBITS, 18,
...
void IN_DrawNumber(int val, int x, int y, int digits) {
  ...
  if (val < 0) { //...this should reflect negative frags
    val = -val;
    neg = true;
    if (val > 99) {
      val = 99;
