...
void S_StartSound(void *_origin, int sound_id) {
  ...
  for (i = 0; i < snd_Channels; i++) {
    ...
    if (origin == channel[i].mo) { // only allow other mobjs one sound
      S_StopSound(channel[i].mo);
      break;
    }
  }
  ...
  channel[i].handle =
      I_StartSound(&S_sfx[sound_id], i, vol, sep, channel[i].pitch);
  channel[i].mo = origin;
...
void S_StartSoundAtVolume(void *_origin, int sound_id, int volume) {
  ...
  channel[i].handle =
      I_StartSound(&S_sfx[sound_id], i, volume, 128, channel[i].pitch);
  channel[i].mo = origin;
  channel[i].sound_id = sound_id;
...
void S_PauseSound(void) { I_PauseSong(); }

void S_ResumeSound(void) { I_ResumeSong(); }
...
void S_UpdateSounds(mobj_t *listener) {
  ...
  for (i = 0; i < snd_Channels; i++) {
    ...
    if (channel[i].mo == NULL || channel[i].sound_id == 0 ||
      ...
      dist = absx + absy - (absx > absy ? absy >> 1 : absx >> 1);
      dist >>= FRACBITS;
      //          dist = P_AproxDistance(channel[i].mo->x-listener->x,
      //          channel[i].mo->y-listener->y)>>FRACBITS;
      ...
      // calculate the volume based upon the distance from the sound origin.
      //          vol = (*((byte *)W_CacheLumpName("SNDCURVE",
      //          PU_CACHE)+dist)*(snd_MaxVolume*8))>>7;
      vol = soundCurve[dist];

      angle = R_PointToAngle2(listener->x, listener->y, channel[i].mo->x,
                              channel[i].mo->y);
      angle = (angle - viewangle) >> 24;
      sep = angle * 2 - 128;
      if (sep < 64)
...
void S_GetChannelInfo(SoundInfo_t *s) {
  ...
  for (i = 0; i < snd_Channels; i++) {
    ...
    if (c->mo != NULL) {
      c->distance =
          P_AproxDistance(c->mo->x - viewx, c->mo->y - viewy) >> FRACBITS;
    } else {
      c->distance = 0;
    }
  }
...
void S_SetMaxVolume(boolean fullprocess) {
  ...
  if (!fullprocess) {
    ...
    for (i = 0; i < MAX_SND_DIST; i++) {
      soundCurve[i] = (*((byte *)W_CacheLumpName("SNDCURVE", PU_CACHE) + i) *
                       (snd_MaxVolume * 8)) >>
                      7;
    }
  }
