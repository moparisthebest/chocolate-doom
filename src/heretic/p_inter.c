...
static weapontype_t GetAmmoChange[] = {wp_goldwand, wp_crossbow,   wp_blaster,
...
boolean P_GiveAmmo(player_t *player, ammotype_t ammo, int count) {
  ...
  if (gameskill == sk_baby ||
      gameskill == sk_nightmare) { // extra ammo in baby mode and nightmare mode
    count += count >> 1;
  }
...
boolean P_GiveWeapon(player_t *player, weapontype_t weapon) {
  ...
  if (netgame && !deathmatch) { // Cooperative net-game
  ...
  if (player->weaponowned[weapon]) {
    ...
    gaveWeapon = true;
    player->weaponowned[weapon] = true;
    if (WeaponValue[weapon] >
        WeaponValue[player->readyweapon]) { // Only switch to more powerful
                                            // weapons
      player->pendingweapon = weapon;
    }
  }
...
boolean P_GiveArtifact(player_t *player, artitype_t arti, mobj_t *mo) {
  ...
  if (i == player->inventorySlotNum) {
    ...
    if (player->inventory[i].count >=
        16) { // Player already has 16 of this item
      return (false);
    }
...
void A_RestoreSpecialThing1(mobj_t *thing) {
  if (thing->type == MT_WMACE) { // Do random mace placement
...
void P_KillMobj(mobj_t *source, mobj_t *target) {
  ...
  if (source && source->player) {
    if (target->flags & MF_COUNTKILL) { // Count for intermission
      source->player->killcount++;
    }
    if (target->player) {     // Frag stuff
      if (target == source) { // Self-frag
        ...
        if (source->player->chickenTics) { // Make a super chicken
          P_GivePower(source->player, pw_weaponlevel2);
        }
      }
    }
  } else if (!netgame &&
             (target->flags & MF_COUNTKILL)) { // Count all monster deaths
    players[0].killcount++;
  }
  if (target->player) {
    if (!source) { // Self-frag
...
boolean P_ChickenMorphPlayer(player_t *player) {
  ...
  if (player->chickenTics) {
    if ((player->chickenTics < CHICKENTICS - TICRATE) &&
        !player->powers[pw_weaponlevel2]) { // Make a super chicken
...
void P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source,
  ...
  if (target->flags & MF_SKULLFLY) {
    if (target->type ==
        MT_MINOTAUR) { // Minotaur is invulnerable during charge attack
      return;
    }
  ...
  if (inflictor) {
    ...
    case MT_MACEFX4: // Death ball
      if ((target->flags2 & MF2_BOSS) ||
          target->type == MT_HEAD) { // Don't allow cheap boss kills
        break;
      } else if (target->player) { // Player specific checks
        if (target->player
                ->powers[pw_invulnerability]) { // Can't hurt invulnerable
                                                // players
          break;
        }
        if (P_AutoUseChaosDevice(
                target->player)) { // Player was saved using chaos device
          return;
        }
      }
    ...
    case MT_PHOENIXFX2:                         // Flame thrower
      if (target->player && P_Random() < 128) { // Freeze player for a bit
    ...
    case MT_RAINPLR3:
    case MT_RAINPLR4:
      if (target->flags2 & MF2_BOSS) { // Decrease damage for bosses
        damage = (P_Random() & 7) + 1;
      }
      break;
    case MT_HORNRODFX2:
    case MT_PHOENIXFX1:
      if (target->type == MT_SORCERER2 &&
          P_Random() < 96) { // D'Sparil teleports away
    ...
    case MT_BLASTERFX1:
    case MT_RIPPER:
      if (target->type == MT_HEAD) { // Less damage to Ironlich bosses
        damage = P_Random() & 1;
        if (!damage) {
          return;
        }
  ...
  if (player) {
    ...
    if (damage >= player->health && ((gameskill == sk_baby) || deathmatch) &&
        !player->chickenTics) { // Try to use some inventory health
  ...
  if (target->health <= 0) { // Death
    ...
    if (target->type == MT_POD && source &&
        source->type !=
            MT_POD) { // Make sure players get frags for chain-reaction kills
      target->target = source;
    }
    if (player && inflictor && !player->chickenTics) { // Check for flame death
