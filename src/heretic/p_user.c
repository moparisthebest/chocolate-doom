...
void P_DeathThink(player_t *player) {
  ...
  onground = (player->mo->z <= player->mo->floorz);
  if (player->mo->type == MT_BLOODYSKULL) { // Flying bloody skull
  ...
  if (player->attacker && player->attacker != player->mo) {
    ...
    if (delta < ANG5 ||
        delta >
            (unsigned)-ANG5) { // Looking at killer, so fade damage flash down
...
void P_ChickenPlayerThink(player_t *player) {
  ...
  if (player->health > 0) { // Handle beak movement
  ...
  pmo = player->mo;
  if (!(pmo->momx + pmo->momy) && P_Random() < 160) { // Twitch view angle
    pmo->angle += P_SubRandom() << 19;
  }
  if ((pmo->z <= pmo->floorz) && (P_Random() < 32)) { // Jump and noise
    pmo->momz += FRACUNIT;
    P_SetMobjState(pmo, S_CHICPLAY_PAIN);
    return;
  }
  if (P_Random() < 48) { // Just noise
...
boolean P_UndoPlayerChicken(player_t *player) {
  ...
  if (playerNum != 0) { // Set color translation
...
void P_PlayerThink(player_t *player) {
  ...
  if (!player->messageTics) { // Refresh the screen when a message goes away
  ...
  // Chicken counter
  if (player->chickenTics) {
    if (player->chickenPeck) { // Chicken attack counter
      player->chickenPeck -= 3;
    }
    if (!--player->chickenTics) { // Attempt to undo the chicken
...
boolean P_UseArtifact(player_t *player, artitype_t arti) {
  ...
  case arti_tomeofpower:
    if (player->chickenTics) {                    // Attempt to undo chicken
      if (P_UndoPlayerChicken(player) == false) { // Failed
