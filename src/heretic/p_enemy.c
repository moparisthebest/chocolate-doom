...
void P_InitMonsters(void) { BossSpotCount = 0; }
...
boolean P_CheckMissileRange(mobj_t *actor) {
  ...
  if (actor->type == MT_IMP) { // Imp's fly attack from far away
    dist >>= 1;
  }
  if (dist > 200) {
    dist = 200;
...
void A_KnightAttack(mobj_t *actor) {
  ...
  // Throw axe
  S_StartSound(actor, actor->info->attacksound);
  if (actor->type == MT_KNIGHTGHOST || P_Random() < 40) { // Red axe
...
void A_ImpExplode(mobj_t *actor) {
  ...
  mo->momy = P_SubRandom() << 10;
  mo->momz = 9 * FRACUNIT;
  if (actor->special1.i == 666) { // Extreme death crash
...
void A_Srcr1Attack(mobj_t *actor) {
  ...
  if (actor->health > (actor->info->spawnhealth / 3) * 2) { // Spit one fireball
    ...
    if (actor->health < actor->info->spawnhealth / 3) { // Maybe attack again
      if (actor->special1.i) { // Just attacked, so don't attack again
        actor->special1.i = 0;
      } else { // Set state to attack again
        actor->special1.i = 1;
...
void A_Srcr2Attack(mobj_t *actor) {
  ...
  if (P_Random() < chance) { // Wizard spawners
...
void A_Sor2DthLoop(mobj_t *actor) {
  if (--actor->special1.i) { // Need to loop
...
void A_MinotaurDecide(mobj_t *actor) {
  ...
  if (target->z + target->height > actor->z &&
    ...
    actor->special1.i = 35 / 2; // Charge duration
             ...
             P_Random() < 220) { // Floor fire attack
...
void A_HeadAttack(mobj_t *actor) {
  ...
  dist = P_AproxDistance(actor->x - target->x, actor->y - target->y) >
         8 * 64 * FRACUNIT;
  randAttack = P_Random();
  if (randAttack < atkResolve1[dist]) { // Ice ball
    P_SpawnMissile(actor, target, MT_HEADFX1);
    S_StartSound(actor, sfx_hedat2);
  } else if (randAttack < atkResolve2[dist]) { // Fire column
...
void A_GhostOff(mobj_t *actor) { actor->flags &= ~MF_SHADOW; }
...
void A_Scream(mobj_t *actor) {
  ...
  case MT_PLAYER:
    ...
    if (actor->special1.i < 10) { // Wimpy death sound
...
void A_MakePod(mobj_t *actor) {
  ...
  if (actor->special1.i == MAX_GEN_PODS) { // Too many generated pods
    return;
  }
  x = actor->x;
  y = actor->y;
  mo = P_SpawnMobj(x, y, ONFLOORZ, MT_POD);
  if (P_CheckPosition(mo, x, y) == false) { // Didn't fit
    P_RemoveMobj(mo);
    return;
  }
...
void A_BossDeath(mobj_t *actor) {
  ...
  thinker_t *think;
  line_t dummyLine;
  static mobjtype_t bossType[6] = {MT_HEAD, MT_MINOTAUR, MT_SORCERER2,
                                   MT_HEAD, MT_MINOTAUR, -1};

  if (gamemap != 8) { // Not a boss level
    return;
  }
  if (actor->type !=
      bossType[gameepisode - 1]) { // Not considered a boss in this episode
    return;
  }
  ...
  for (think = thinkercap.next; think != &thinkercap; think = think->next) {
    ...
    if ((mo != actor) && (mo->type == actor->type) &&
        (mo->health > 0)) { // Found a living boss
      return;
    }
  }
  if (gameepisode > 1) { // Kill any remaining monsters
    P_Massacre();
  }
  dummyLine.tag = 666;
...
void A_VolcanoSet(mobj_t *volcano) { volcano->tics = 105 + (P_Random() & 127); }
...
void A_VolcanoBlast(mobj_t *volcano) {
  ...
  for (i = 0; i < count; i++) {
    blast = P_SpawnMobj(volcano->x, volcano->y, volcano->z + 44 * FRACUNIT,
                        MT_VOLCANOBLAST); // MT_VOLCANOBLAST
