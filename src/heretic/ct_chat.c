...
char plr_lastmsg[MAXPLAYERS]
                [MESSAGESIZE + 9]; // add in the length of the pre-string
  ...
  if (!chatmodeon) {
    ...
    if (altdown) {
      if (ev->data1 >= '0' && ev->data1 <= '9') {
        if (ev->data1 == '0') { // macro 0 comes after macro 9
          ev->data1 = '9' + 1;
        }
    ...
    if ((c = players[i].cmd.chatchar) != 0) {
      if (c <= 5) {
        ...
        if (numplayers > 2) {
          ...
          M_StringCopy(plr_lastmsg[i], chat_msg[i], sizeof(plr_lastmsg[i]));
        }
        if (i != consoleplayer &&
          ...
          if (numplayers > 1) {
            P_SetMessage(&players[consoleplayer], DEH_String("-MESSAGE SENT-"),
                         true);
            S_StartSound(NULL, sfx_chat);
          } else {
  ...
  if (chatmodeon) {
    ...
    for (i = 0; i < msgptr[consoleplayer]; i++) {
      if (chat_msg[consoleplayer][i] < 33) {
        ...
        patch = W_CacheLumpNum(FontABaseLump + chat_msg[consoleplayer][i] - 33,
                               PU_CACHE);
...
void CT_queueChatChar(char ch) {
  if (((tail + 1) & (QUEUESIZE - 1)) == head) { // the queue is full
    return;
  }
  ChatQueue[tail] = ch;
...
void CT_BackSpace(int player) {
  ...
  if (msgptr[player] == 0) { // message is already blank
    return;
  }
  msgptr[player]--;
