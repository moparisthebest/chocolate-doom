...
static Cheat_t Cheats[] = {{CheatGodFunc, &CheatGodSeq},
...
void SB_Init(void) {
  ...
  if (!netgame) { // single player game uses red life gem
    PatchLIFEGEM = W_CacheLumpName(DEH_String("LIFEGEM2"), PU_STATIC);
  } else {
    PatchLIFEGEM = W_CacheLumpNum(
        W_GetNumForName(DEH_String("LIFEGEM0")) + consoleplayer, PU_STATIC);
  }
...
void SB_Drawer(void) {
    ...
    if (CPlayer->powers[pw_flight] > BLINKTHRESHOLD ||
      ...
      if (CPlayer->mo->flags2 & MF2_FLY) {
        if (hitCenterFrame && (frame != 15 && frame != 0)) {
          V_DrawPatch(20, 17, W_CacheLumpNum(spinflylump + 15, PU_CACHE));
        } else {
          V_DrawPatch(20, 17, W_CacheLumpNum(spinflylump + frame, PU_CACHE));
  ...
  /*
                  ...
                  {
                          V_DrawPatch(291, 0, W_CacheLumpName("ARTIPWBK",
     PU_CACHE));
                  }
                  else
                  {
  ...
  if (!inventory) {
    ...
    for (i = 0; i < 7; i++) {
      ...
      if (CPlayer->inventorySlotNum > x + i &&
          CPlayer->inventory[x + i].type != arti_none) {
        patch = DEH_String(patcharti[CPlayer->inventory[x + i].type]);
        V_DrawPatch(50 + i * 31, 168, W_CacheLumpName(patch, PU_CACHE));
...
static boolean HandleCheats(byte key) {
  ...
  if (netgame ||
      gameskill ==
          sk_nightmare) { // Can't cheat in a net-game, or in nightmare mode
    return (false);
  }
...
static void CheatArtifact3Func(player_t *player, Cheat_t *cheat) {
  ...
  type = args[0] - 'a' + 1;
  count = args[1] - '0';
  if (type == 26 && count == 0) { // All artifacts
  ...
  } else if (type > arti_none && type < NUMARTIFACTS && count > 0 &&
             count < 10) {
    if (gamemode == shareware &&
