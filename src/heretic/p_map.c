...
boolean PIT_CheckLine(line_t *ld) {
  ...
  */
  ...
  if (!ld->backsector) {               // One sided line
    if (tmthing->flags & MF_MISSILE) { // Missiles can trigger impact specials
      if (ld->special) {
  ...
  if (!(tmthing->flags & MF_MISSILE)) {
    ...
    if (!tmthing->player && ld->flags & ML_BLOCKMONSTERS &&
        tmthing->type != MT_POD) { // Block monsters only
      return (false);
    }
  }
...
boolean PIT_CheckThing(mobj_t *thing) {
  ...
  if (tmthing->flags2 &
    ...
    if ((tmthing->type == MT_IMP || tmthing->type == MT_WIZARD) &&
        (thing->type == MT_IMP ||
         thing->type ==
             MT_WIZARD)) { // don't let imps/wizards fly over other imps/wizards
      return false;
    }
  ...
  if (tmthing->flags & MF_MISSILE) {
    ...
    if (tmthing->target &&
      ...
      if (thing->type !=
          MT_PLAYER) { // Hit same species as originator, explode, no damage
        return (false);
      }
    }
    ...
    if (tmthing->flags2 & MF2_RIP) {
      if (!(thing->flags & MF_NOBLOOD)) { // Ok to spawn some blood
...
floorz
ceilingz
tmdropoffz		the lowest point contacted (monsters won't move to a
dropoff) speciallines[] numspeciallines
...
boolean P_TryMove(mobj_t *thing, fixed_t x, fixed_t y) {
  ...
  if (!(thing->flags & MF_NOCLIP)) {
    ...
    if (!(thing->flags & MF_TELEPORT)
        ...
        tmfloorz - thing->z > 24 * FRACUNIT) { // Too big a step up
      CheckMissileImpact(thing);
      return false;
    }
    ...
    if (!(thing->flags & (MF_DROPOFF | MF_FLOAT)) &&
        tmfloorz - tmdropoffz > 24 * FRACUNIT) { // Can't move over a dropoff
      return false;
    }
  }
...
retry:
  ...
  //
  if (bestslidefrac ==
      FRACUNIT + 1) { // the move most have hit the middle, so stairstep
  stairstep:
...
boolean PTR_AimTraverse(intercept_t *in) {
  ...
  if (th->type == MT_POD) { // Can't auto-aim at pods
    return (true);
  }
...
boolean PTR_ShootTraverse(intercept_t *in) {
  ...
  y = trace.y + FixedMul(trace.dy, frac);
  z = shootz + FixedMul(aimslope, FixedMul(frac, attackrange));
  if (PuffType == MT_BLASTERPUFF1) { // Make blaster big puff
...
boolean PIT_RadiusAttack(mobj_t *thing) {
  ...
  if (thing->type == MT_MINOTAUR || thing->type == MT_SORCERER1 ||
      thing->type == MT_SORCERER2) { // Episode 2 and 3 bosses take no damage
                                     // from PIT_RadiusAttack
    return (true);
  }
  ...
  if (dist >= bombdamage) { // Out of range
    return true;
  }
...
boolean PIT_ChangeSector(mobj_t *thing) {
  ...
  if (crushchange && !(leveltime & 3)) {
    ...
    mo =
        P_SpawnMobj(thing->x, thing->y, thing->z + thing->height / 2, MT_BLOOD);
