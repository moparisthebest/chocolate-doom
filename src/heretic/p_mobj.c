  ...
  do {
    ...
    if (!P_TryMove(mo, ptryx, ptryy)) { // Blocked move
      if (mo->flags2 & MF2_SLIDE) {     // Try to slide along it
        P_SlideMove(mo);
      } else if (mo->flags & MF_MISSILE) { // Explode a missile
    ...
    if (mo->flags & MF_MISSILE) {
      ...
      if (mo->flags2 & MF2_FLOORBOUNCE) {
        ...
        return;
                 ...
                 MT_MNTRFX2) { // Minotaur floor fire can go up steps
        return;
      } else {
        P_ExplodeMissile(mo);
        return;
...
void P_MobjThinker(mobj_t *mobj) {
  ...
  if (mobj->flags2 & MF2_FLOATBOB) { // Floating item bobbing motion
    mobj->z = mobj->floorz + FloatBobOffsets[(mobj->health++) & 63];
  } else if ((mobj->z != mobj->floorz) ||
             mobj->momz) { // Handle Z momentum and gravity
  ...
  } else { // Check for monster respawn
...
void P_SpawnMapThing(mapthing_t *mthing) {
  ...
  case MT_ITEMSHIELD2:
    if (gamemode == shareware) { // Don't place on map in shareware version
      return;
    }
    break;
  case MT_WMACE:
    if (gamemode != shareware) { // Put in the mace spot list
...
mobj_t *P_SpawnPlayerMissile(mobj_t *source, mobjtype_t type) {
  ...
  if (MissileMobj->type ==
      MT_BLASTERFX1) { // Ultra-fast ripper spawning missile
