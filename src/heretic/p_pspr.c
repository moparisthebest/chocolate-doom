...
void P_OpenWeapons(void) { MaceSpotCount = 0; }
...
void P_CloseWeapons(void) {
  ...
  if (!MaceSpotCount) { // No maces placed
    return;
  }
  if (!deathmatch &&
      P_Random() < 64) { // Sometimes doesn't show up if not in deathmatch
    return;
  }
  spot = P_Random() % MaceSpotCount;
...
void P_PostChickenWeapon(player_t *player, weapontype_t weapon) {
  if (weapon == wp_beak) { // Should never happen
    weapon = wp_staff;
  }
  player->pendingweapon = wp_nochange;
...
boolean P_CheckAmmo(player_t *player) {
  ...
  if (player->powers[pw_weaponlevel2]) {
    P_SetPsprite(player, ps_weapon, wpnlev2info[player->readyweapon].downstate);
  } else {
    P_SetPsprite(player, ps_weapon, wpnlev1info[player->readyweapon].downstate);
  }
  return (false);
}
...
void P_FireWeapon(player_t *player) {
  ...
  wpinfo = player->powers[pw_weaponlevel2] ? &wpnlev2info[0] : &wpnlev1info[0];
  ...
  if (player->readyweapon == wp_gauntlets &&
      !player->refire) { // Play the sound for the initial gauntlet attack
...
void A_WeaponReady(player_t *player, pspdef_t *psp) {
  ...
  if (player->chickenTics) { // Change to the chicken beak
...
void A_BeakReady(player_t *player, pspdef_t *psp) {
  if (player->cmd.buttons & BT_ATTACK) { // Chicken beak attack
    player->attackdown = true;
    ...
    if (player->mo->state ==
        &states[S_CHICPLAY_ATK1]) { // Take out of attack state
...
void A_MaceBallImpact(mobj_t *ball) {
  if ((ball->z <= ball->floorz) &&
      (P_HitFloor(ball) != FLOOR_SOLID)) { // Landed in some sort of liquid
    P_RemoveMobj(ball);
    return;
  }
...
void A_MaceBallImpact2(mobj_t *ball) {
  ...
  if ((ball->z != ball->floorz) || (ball->momz < 2 * FRACUNIT)) { // Explode
    ...
    angle >>= ANGLETOFINESHIFT;
    tiny->momx =
        (ball->momx >> 1) + FixedMul(ball->momz - FRACUNIT, finecosine[angle]);
    tiny->momy =
        (ball->momy >> 1) + FixedMul(ball->momz - FRACUNIT, finesine[angle]);
    tiny->momz = ball->momz;
...
void A_DeathBallImpact(mobj_t *ball) {
  ...
  if ((ball->z <= ball->floorz) && ball->momz) { // Bounce
    newAngle = false;
    ...
    if (target) {
      ...
      for (i = 0; i < 16; i++) {
        ...
        if (linetarget && ball->target != linetarget) {
          ...
          angle =
              R_PointToAngle2(ball->x, ball->y, linetarget->x, linetarget->y);
          newAngle = true;
          break;
        }
        angle += ANG45 / 2;
...
void A_AddPlayerRain(mobj_t *actor) {
  ...
  if (!playeringame[playerNum]) { // Player left the game
    return;
  }
  ...
  if (player->rain1 && player->rain2) { // Terminate an active rain
...
void A_SkullRodStorm(mobj_t *actor) {
  ...
  if (P_Random() < 25) { // Fudge rain frequency
    return;
  }
...
void A_FirePhoenixPL2(player_t *player, pspdef_t *psp) {
  ...
  if (--player->flamecount == 0) { // Out of flame
...
void A_FlameEnd(mobj_t *actor) { actor->momz += (fixed_t)(1.5 * FRACUNIT); }
...
void A_FloatPuff(mobj_t *puff) { puff->momz += (fixed_t)(1.8 * FRACUNIT); }
