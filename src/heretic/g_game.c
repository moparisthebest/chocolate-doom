...
void G_BuildTiccmd(ticcmd_t *cmd, int maketic) {
  ...
  speed = joybspeed >= MAX_JOY_BUTTONS || gamekeydown[key_speed] ||
  ...
  if (gamestate == GS_LEVEL && players[consoleplayer].chickenTics == 0 &&
      next_weapon != 0) {
...
void G_DoCompleted(void) {
  ...
  if (secretexit == true) {
    gamemap = 9;
  } else if (gamemap == 9) { // Finished secret level
...
void G_DoLoadGame(void) {
  ...
  if (strncmp(readversion, vcheck, VERSIONSIZE) != 0) { // Bad version
    return;
  }
  ...
  if (SV_ReadByte() !=
      SAVE_GAME_TERMINATOR) { // Missing savegame termination marker
...
void G_InitNew(skill_t skill, int episode, int map) {
  int i;
  int speed;
  static const char *skyLumpNames[5] = {"SKY1", "SKY2", "SKY3", "SKY1", "SKY3"};
