...
char *SV_Filename(int slot) {
  ...
  M_snprintf(filename, filename_len, "%s" SAVEGAMENAME "%d.hsg", savegamedir,
             slot);
...
void SV_Open(char *fileName) { SaveGameFP = fopen(fileName, "wb"); }
...
void SV_Write(void *buffer, int size) { fwrite(buffer, size, 1, SaveGameFP); }
...
void SV_Read(void *buffer, int size) {
  ...
  if (retval != size) {
    I_Error("Incomplete read in SV_Read: Expected %d, got %d bytes", size,
            retval);
  }
}
