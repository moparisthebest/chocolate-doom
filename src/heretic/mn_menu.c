...
static Menu_t EpisodeMenu = {80,           50, DrawEpisodeMenu, 3,
                             EpisodeItems, 0,  MENU_MAIN};
...
static MenuItem_t SkillItems[] = {
    ...
    {ITT_EFUNC, "BLACK PLAGUE POSSESSES THEE", SCSkill, sk_nightmare,
     MENU_NONE}};

static Menu_t SkillMenu = {38,         30, DrawSkillMenu, 5,
                           SkillItems, 2,  MENU_EPISODE};
...
static Menu_t *Menus[] = {&MainMenu,     &EpisodeMenu, &SkillMenu, &OptionsMenu,
...
void MN_Init(void) {
  ...
  if (gamemode == retail) { // Add episodes 4 and 5 to the menu
...
static void DrawMainMenu(void) {
  ...
  V_DrawPatch(40, 10, W_CacheLumpNum(SkullBaseLump + (17 - frame), PU_CACHE));
...
void MN_DrawInfo(void) {
  I_SetPalette(W_CacheLumpName("PLAYPAL", PU_CACHE));
  V_DrawRawScreen(
      W_CacheLumpNum(W_GetNumForName("TITLE") + InfoType, PU_CACHE));
...
static void DrawSlider(Menu_t *menu, int item, int width, int slot) {
  ...
  for (x2 = x, count = width; count--; x2 += 8) {
    V_DrawPatch(x2, y,
                W_CacheLumpName(DEH_String(count & 1 ? "M_SLDMD1" : "M_SLDMD2"),
                                PU_CACHE));
  }
